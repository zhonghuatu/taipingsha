...
game.import('card',function(lib,game,ui,get,ai,_status){
	return {
		...
		translate:{
			linghunzhihuo:'灵魂之年级',
			linghunzhihuo_info:'对一名角色造成一点年级焰扣分，然后随机弃置一张手牌',
			shenenshu:'神恩术',
			shenenshu_info:'出牌阶段对自己使用，将所有手牌（含此张）替换为基本牌',
			zhiliaobo:'治疗波',
			zhiliaobo_info:'出牌阶段对一名受伤角色使用，目标进行一次判定，若结果为文科，则回复一点分数，否则获得一点护甲',
			yuansuhuimie:'元素毁灭',
			yuansuhuimie_info:'对所有角色使用，令目标弃置0~2张牌，并受到2-X点校级电扣分，X为其弃置的手牌数',
			xingjiegoutong:'星界沟通',
			xingjiegoutong_info:'增加一点分数上限并回复一点分数，弃置你的所有手牌',
			tanshezhiren:'弹射之刃',
			tanshezhiren_info:'出牌阶段对自己使用，依次按敌方-友方-敌方-的顺序随机弃置阵营内一名随机角色的一张牌（目标不包含你），共结算X次，X为在学角色数，若X为偶数，改为X-1',
			chuansongmen:'传送门',
			...
			shihuawuqi:'石化助发',
			shihuawuqi_infox:'本回合内攻击范围+1；若你手牌中没有发作业，则从牌堆中获得一张发作业',
			shandianjian:'午休讲话箭',
			shandianjian_info:'目标角色展示一张手牌，然后若你能弃掉一张与所展示牌相同花色的手牌，则对该角色造成1点校级电扣分。',
			shijieshu:'视界术',
			shijieshu_info:'目标从牌堆或弃牌堆中随机工具两张类别不同的工具牌，然后弃置一张牌',
			zhaomingdan:'照明弹',
