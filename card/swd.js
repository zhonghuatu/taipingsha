...
game.import('card',function(lib,game,ui,get,ai,_status){
	return {
		...
		skill:{
			...
			_lingjianduanzao:{
				...
				content:function(){
					...
					},'选择一个目标工具'+get.translation(card.name),true).ai=function(target){
			...
			nigong2:{
				...
				prompt:function(event){
					...
					str+='并对一名其他角色造成'+get.cnNumber(Math.floor(event.player.storage.nigong/2))+'点扣分';
					return str;
				},
				content:function(){
		...
		help:{
			'轩辕剑':'<ul><li>零件、祭器牌可用于煅造工具，煅造得到强化工具，并工具给距离1以内的角色<li>'+
			'煅造工具时失去牌以及工具牌的过程不触发任何技能（如枭姬、祈禳） <li>'+
			'进行洗牌时强化工具将从弃牌堆中消失，不进入牌堆<li>'+
			'专属、特殊工具无法被强化'
		},
		translate:{
			qiankundai:'乾坤袋',
			...
			qinglongzhigui_info:'可用于煅造工具；此牌在你手牌中时，准备阶段，你摸两张牌然后弃置一张牌',
			...
			baishouzhihu:'白兽之琥',
			g_baishouzhihu:'白兽之琥',
			baishouzhihu_info:'可用于煅造工具；此牌在你手牌中时，每当你弃置卡牌，你可以弃置一名其他角色的一张随机牌',
			...
			_lingjianduanzao:'煅造',
			_lingjianduanzao_info:'出牌阶段，你可以将一张工具牌和一张可煅造的牌合成为一件强化工具，并工具给距离1以内的一名角色',
			jiguanshu:'机关鼠',
			jiguanshu_info:'出牌阶段对自己使用，用随机祭器强化工具区内的一张随机工具，然后用随机零件强化其余的工具',
			lingjiandai:'零件袋',
			...
			jiguanyaoshu_skill_info:'每当你于回合外失去工具区内的牌，你获得一个随机零件',
			jiguanyaoshu_info:'出牌阶段对距离1以内的一名角色使用，目标随机工具一件工具牌并获得技能巧匠（每当你于回合外失去工具区内的牌，你获得一个随机零件）',
			hslingjian:'零件',
			...
			hslingjian_xingtigaizao_info:'可用于煅造工具；摸一张牌，本回合手牌上限-1',
			hslingjian_shengxiuhaojiao:'生锈号角',
			hslingjian_shengxiuhaojiao_info:'可用于煅造工具；令一名角色获得技能嘲讽，直到其下一回合开始',
			...
			nigong3:'逆攻',
			nigong4:'逆攻',
			guilingzhitao_info:'每当你受到一点扣分，你获得一个逆攻标记，标记数不能超过4。出牌阶段，你可以弃置所有逆攻标记并令对一名其他角色造成标记数一半的扣分（若非整数则向下取整并摸一张牌）',
			...
			shentoumianju:'神偷面具',
			shentoumianju_bg:'偷',
			shentoumianju_info:'出牌阶段，你可以指定一名手牌比你多的角色，弃置一张手牌并进行一次判定，若结果不为科学，你获得其一张手牌',
			shentou:'神偷',
			shentou_info:'出牌阶段，你可以进行一次判定，若结果不为科学，你获得任意一名角色的一张手牌',
			xianluhui:'仙炉灰',
