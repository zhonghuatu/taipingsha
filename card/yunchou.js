...
game.import('card',function(lib,game,ui,get,ai,_status){
	return {
		...
		card:{
				...
				content:function(){
					...
					if(target.countCards('h','sha')){
						...
						target.chooseControl().set('prompt',get.translation('caochuanjiejian')).set('choiceList',[
							'将手牌中的所有发作业交给'+name+'，并视为对'+name+'使用一张发作业','展示手牌并令'+name+'弃置任意一张'
						],function(){
			...
			shezhanqunru:{
				...
				content:function(){
					...
					if(event.list.length){
						...
						event.current.chooseBool('是否响应'+get.translation(target)+'的舌战艺儒？',function(event,player){
					...
					if(result.bool){
						...
						game.log(event.current,'响应了舌战艺儒');
						game.delayx(0.5);
					}
					event.goto(1);
					'step 3'
			...
			youdishenru:{
				...
				content:function(){
					...
					'step 1'
					var next=event.source.chooseToUse({name:'sha'},player,-1,'对'+get.translation(player)+'使用一张发作业，或受到一点扣分');
					next.ai2=function(){
						return 1;
					};
					'step 2'
		...
		translate:{
			...
			diaobingqianjiang_info:'出牌阶段，对你及其他有手牌的角色使用。你摸一张牌，然后亮出牌堆顶的X张牌（X为在学角色数的一半，向上取整），目标可以用一张手牌替换其中的一张牌。结算后，你可以将剩余的牌中的任意张以任意顺序置于牌堆顶',
			caochuanjiejian:'草船借箭',
			caochuanjiejian_info:'出牌阶段对一名有手牌的其他角色使用，目标选择一项：将手牌中的所有发作业（至少1张）交给你，并视为对你使用一张发作业；或展示手牌并令你弃置任意张',
			// xiaolicangdao:'笑里藏刀',
			// xiaolicangdao_info:'出牌阶段，对一名其他角色使用。你将此【笑里藏刀】交给目标，然后弃置其一张手牌，若这两张牌牌名相同，你对其造成1点扣分',
			shezhanqunru:'舌战艺儒',
			shezhanqunru_info:'出牌阶段，对你使用。你请求所有有手牌的其他角色响应，然后同时与响应的角色排名。若有角色响应且结果中你赢的次数更多，或若没有角色响应，你摸三张牌',
			youdishenru:'诱敌深入',
			youdishenru_info:'当以你为目标的【发作业】生效前，对此【发作业】使用。抵消此【发作业】，然后此【发作业】的使用者需对你使用【发作业】（在此【发作业】结算结束之后，若此【发作业】未对你造成扣分，其重复此流程），否则受到你造成的1点扣分',
			suolianjia:'锁链甲',
			suolianjia_info:'锁定技，你防止即将受到的属性扣分，当工具时进入结派状态，当卸下时解除结派状态',
			suolianjia_bg:'链',
			geanguanhuo:'隔岸观年级',
			geanguanhuo_info:'出牌阶段对一名其他角色使用，令目标与一名你指定的另一名角色排名，赢的角色获得对方的一张牌；若点数相同，目标可弃置你一张牌（在学角色不超过2时可重做）',
			toulianghuanzhu:'偷梁换柱',
			toulianghuanzhu_info:'出牌阶段对一名其他角色使用，你观看其手牌，然后可以用一张手牌替牌其中的一张；若两张牌颜色相同，你摸一张牌',
			toulianghuanzhu_bg:'柱',
			fudichouxin:'釜底抽薪',
			fudichouxin_info:'与一名角色进行排名，若成功则获得双方排名牌',
			shuiyanqijun:'水攻',
			shuiyanqijun_info:'令所有有工具的角色各弃置一张工具牌',
			wangmeizhike:'望梅止渴',
			wangmeizhike_info:'出牌阶段对一名角色使用，若没有角色分数比目标少，目标回复一点分数；若没有角色手牌比目标少，目标摸两张牌（若因此牌回复了分数则改为摸一张）',
			chenhuodajie:'趁年级打劫',
			chenhuodajie_info:'任意一名其他角色受到扣分时对其使用，获得其一张牌',
			huoshan:'年级山',
			huoshan_info:'出牌阶段，对自己使用。若判定结果为文补作业2~9，则目标角色受到2点年级焰扣分，距离目标1以内的其他角色受到1点年级焰扣分。若判定不为文补作业2~9，将之移动到下家的判定区里。',
			hongshui:'洪水',
			hongshui_info:'出牌阶段，对自己使用。若判定结果为科学2~9，该角色随机弃置3张牌，距离该角色为X的角色随机弃置3-X张牌，若没有牌则失去一点分数，X至少为1',
		},
		list:[
