...
game.import('card',function(lib,game,ui,get,ai,_status){
	return {
		...
		card:{
				...
				content:function(){
					...
					if(target.countCards('h','sha')){
						...
						target.chooseControl().set('prompt',get.translation('caochuanjiejian')).set('choiceList',[
							'将手牌中的所有问交给'+name+'，并视为对'+name+'使用一张问','展示手牌并令'+name+'弃置任意一张'
						],function(){
			...
			youdishenru:{
				...
				content:function(){
					...
					'step 1'
					var next=event.source.chooseToUse({name:'sha'},player,-1,'对'+get.translation(player)+'使用一张问，或受到一点扣分');
					next.ai2=function(){
						return 1;
					};
					'step 2'
		...
		translate:{
			...
			caochuanjiejian_info:'出牌阶段对一名有手牌的其他角色使用，目标选择一项：将手牌中的所有问（至少1张）交给你，并视为对你使用一张问；或展示手牌并令你弃置任意张',
			// xiaolicangdao:'笑里藏刀',
			...
			youdishenru:'诱敌深入',
			youdishenru_info:'当以你为目标的【问】生效前，对此【问】使用。抵消此【问】，然后此【问】的使用者需对你使用【问】（在此【问】结算结束之后，若此【问】未对你造成扣分，其重复此流程），否则受到你造成的1点扣分',
			suolianjia:'锁链甲',
			...
			huoshan:'火山',
			huoshan_info:'出牌阶段，对自己使用。若判定结果为语文2~9，则目标角色受到2点文竞扣分，距离目标1以内的其他角色受到1点文竞扣分。若判定不为语文2~9，将之移动到下家的判定区里。',
			hongshui:'洪水',
			hongshui_info:'出牌阶段，对自己使用。若判定结果为科学2~9，该角色随机弃置3张牌，距离该角色为X的角色随机弃置3-X张牌，若没有牌则失去一点体力',
		},
		list:[
