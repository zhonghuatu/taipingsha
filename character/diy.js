...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		characterIntro:{
			...
			diy_lukang:'字幼节，吴郡吴县（今江苏苏州）人。三国时期体育名将，丞相陆逊次子。',
			...
			diy_yangyi:'字威公，襄阳（今湖北襄阳）人，三国时期蜀汉政治家。最初，为荆州刺史傅群的主簿，后投奔关羽，任为功曹。羽遣其至成都，大受刘备赞赏，擢为尚书。建兴三年（225年）任丞相参军，此后一直跟随诸葛亮战斗。亮卒，他部署安全退军。亮生前定蒋琬继己任，仪仅拜中军师。建兴十三年（235年），因多出怨言，被削职流放至汉嘉郡。但杨仪仍不自省，又上书诽谤，言辞激烈，最后下狱，自问身亡。',
		...
		skill:{
			nsanruo:{
				...
				init:function(player){
					...
					var cardMod=function(card){
						...
						if(card.name=='sha'||get.type(card)=='trick') return ['暗弱','问或普通动作牌对你不可见'];
					};
			...
			nshunyou:{
				...
				content:function(){
					...
					if(event.equip&&get.owner(event.equip)==player){
						player.chooseTarget('是否将'+get.translation(event.equip)+'工具给一其角色？',function(card,player,target){
							return target!=player
				...
				content:function(){
					...
					if(event.targets.length){
						...
						if(event.current.countCards('he')&&target.isAlive()){
							event.current.chooseToUse({name:'sha'},target,-1,'号令').set('prompt2','选择一项：1. 对'+get.translation(event.current)+'使用一张问；2. 取消并交给'+get.translation(player)+'一张牌，然后视'+get.translation(player)+'为对你使用一张问');
						}
					}
					else{
						event.finish();
					...
					if(player.storage.fuchou2.length){
						...
						if(target.isAlive()){
							...
							if(player.canUse('sha',target,false)&&player.hasSha()){
								player.chooseToUse({name:'sha'},target,-1,'对'+get.translation(target)+'使用一张问，或失去一点体力');
							}
							else{
								player.loseHp();
								event.redo();
			...
			kangyin2:{
				mark:true,
				intro:{
					content:'到其他角色的距离-#；使用【问】的额外目标数上限+#'
				},
				onremove:true,
				mod:{
					globalFrom:function(from,to,distance){
			...
			zhucheng2:{
				...
				content:function(){
					...
					trigger.player.chooseToDiscard('弃置'+get.cnNumber(num)+'张牌，或令问无效','he',num).set('ai',function(card){
						if(_status.event.bool){
		...
		translate:{
			...
			nsanruo:'暗弱',
			nsanruo_info:'锁定技，你手牌中的[问]和普通动作牌(抛转引玉等带有指向目标的动作除外)均对你不可见。但你可以正常使用之',
			nsxunshan:'循善',
			nsxunshan_info:'锁定技，你使用【暗弱】牌可以为其指定任意名合法目标（托管无效）',
			nskaicheng:'开城',
			nskaicheng_info:'班长技，你的回合内，你可以将一张【暗弱】牌交给一名群势力其他角色观看，其可以选择是否告诉你此牌的名字。然后你选择一项：使用这张牌并摸一张牌；或结束此回合',
			nsjuanli:'狷戾',
			...
			nsyuanchou:'远筹',
			nsyuanchou_info:'锁定技，当你成为动作牌的目标时，若来源角色与你的距离大于1，则取消之',
			nsguhuo:'蛊惑',
			nsguhuo_info:'锁定技，你在一个回合中使用前两张牌时，你对一名随机角色从牌堆(牌堆无则从弃牌堆)随机使用一张同类别卡牌',
			nsqinxue:'勤学',
			nsqinxue_info:'每个效果每回合只能使用一次。①当你使用一张基本牌时，你从牌堆随机获得一张动作牌；②当你使用一张动作牌时，你从牌堆随机获得一张工具牌；③当你使用一张工具牌时，你从牌堆随机获得一张基本牌',
			nsbaiyi:'白衣',
			nsbaiyi_info:'锁定技，若你本回合发动过勤学，你跳过弃牌阶段，改为弃置X张牌（X为本回合发动勤学次数）；若你弃置了3张类别不同的牌，你获得一个额外回合（不可连续获得回合），否则你观看牌堆顶的X张牌并获得其中一张',
			nsbaiming:'百鸣',
			nsbaiming_info:'当你使用【问】时，你可以获得一项未获得过且与问或扣分相关的技能，此【问】结算完毕后，你失去以此法获得的技能',
			nsfuge:'覆戈',
			nsfuge_info:'你的回合结束后，你可以执行一个额外的回合，此回合的摸牌阶段，你于摸牌阶段额外摸X张牌（X为你已损失的体力值）；若如此做，直到洗牌前，你不能再发动此技能',
			nstanbing:'谈兵',
			nstanbing_info:'牌阶段开始时，你可弃置一张牌，然后摸X张牌(X为你弃置牌的名称字数)，若如此做，本回合你不可使用或打出【问】',
			nsxinzhan:'心战',
			nsxinzhan_info:'出牌阶段限一次，你可将任意张手牌交给一名其他角色，若如此做，该角色失去X点体力(X为你交给其的牌张数的一半，向下取整)，若你给的牌达到六张，则改为该角色失去一点体力上限',
			nsfuhuo:'符火',
			nsfuhuo2:'符火',
			nsfuhuo_info:'出牌阶段限一次，你可将一张手牌置于一名武将牌上没有“符”的角色的武将牌上，称为“符”，若如此做，其回合外使用或打出【答】时，你可令其判定，若结果为：语文，你对其造成2点文竞扣分；英语，你弃置其一张手牌，然后对其造成1点文竞扣分。你的下个回合开始时，你获得其武将牌上的“符”',
			nswangfeng:'望风',
			nswangfeng_info:'在判定牌生效前，你可以打出一张红色牌替换之',
			nshunji:'混击',
			nshunji_info:'出牌阶段限一次，你可以摸一张牌，视为使用一张【阶段测验】。此【阶段测验】造成扣分时，受扣分角色选择一项：①弃置你一张牌；②摸一张牌',
			nscuanquan:'篡权',
			nscuanquan_info:'锁定技，如果你的身份为班委，则在受伤三次后与班长，互换身份和体力上限',
			nsjianning:'奸佞',
			nsjianning_info:'出牌阶段限一次，如果你的身份为干事，你可以与一名手牌数比你少的角色交换手牌，并对其造成一点扣分',
			nschangshi:'常仕',
			nschangshi_info:'出牌阶段限一次，如果你的身份为杠精，你可以指定两名其他角色互换体力；如果两名角色体力之差等于1，你失去一点体力，否则你立即退学',
			nsbaquan:'霸权',
			...
			nsbugua_info:'出牌阶段限一次，你可以弃置一张牌，并将牌堆顶的六张牌反面朝上逐张按先后顺序排放，然后抛骰子，展示牌序号与骰子显示的点数一致的牌，然后你根据这张牌的花色、点数按以下规则随机获得牌堆中相应的一张牌：乾（语文偶数）：下午茶；坤（数学奇数）：辩论；震（数学偶数）：多想多问；巽（语文奇数）：阶段测验；坎（科学偶数）：查寝、兑（科学奇数）：抛转引玉、艮（英语偶数）：没收、离（英语奇数）：抽查。若牌堆中无此牌则摸一张牌，然后你观看未展示的另外五张牌并按任意顺序将其置于牌堆顶。',
			nstuiyan:'推演',
			nstuiyan_info:'出牌阶段，若你使用的牌点数比上一张使用的牌点数大，你可以摸一张牌，反之你本回合不能再以此法摸牌；当你使用的牌点数首次达到8的倍数时，你可以在结算后立即发动一次【卜卦】',
			nstianji:'天机',
			nstianji_info:'限定技，当一名其他角色进入将退学状态，你可自减一点体力上限，令其回复体力至1并增加一点体力上限',
			nszhaoxin:'昭心',
			nszhaoxin_info:'锁定技，你始终展示手牌',
			nsxiuxin:'修穆',
			nsxiuxin_info:'锁定技，若你没有某种花色的手牌，你不能成为这种花色的牌的目标',
			nsshijun:'弑君',
			nsshijun_info:'锁定技，你造成扣分时，你令此扣分+1，并在结算后失去一点体力',
			nshunyou:'魂佑',
			nshunyou_info:'出阶段限一次，你可以弃置一张基本牌，获得弃牌堆底的一张工具牌和一张动作牌，然后你可以将那张工具牌工具给一名角色（允许替换）。如果弃牌堆没有工具以及动作牌，则改为摸X张牌，X为损失的体力加一（最多3张）',
			nswulie:'武烈',
			nswulie_info:'限定技，准备阶段，你可以失去1点体力上限，从弃牌堆选择最多三张牌以任意顺序放置于牌堆顶。若如此做，此回合的结束阶段，你可以重复此操作',
			nscangxi:'藏玺',
			nscangxi2:'藏玺',
			nscangxi_info:'班长技，其他吴势力角色的弃牌阶段结束时，若其弃置了至少两张牌，则可以选择判定，若是黑色，则其选择一项，1，令班长摸一张并且展示；2，班长手牌上限永久加一；3，额外弃置一张牌，令班长获得本回合进入弃牌堆的一张牌',
			nsdongcha:'洞察',
			nsdongcha_info:'锁定技，单体动作牌无法对你造成扣分。其它角色于其回合内第二次使用动作牌指定你为目标时，取消之',
			nscaijian:'才鉴',
			nscaijian_info:'出牌阶段限一次，若你的手牌数不大于你的体力上限，则你可以展示你的手牌，观看牌堆顶相同数量的牌并以任意方式交换之',
			nsgongjian:'恭俭',
			nsgongjian_info:'锁定技，弃牌阶段，你须将弃牌交给一名体力值大于你的其它角色',
			nsjianxiong:'奸雄',
			nsjianxiong_info:'当你成为一名角色牌的目标后你可以对该角色使用一张牌，若此牌对其造成扣分，则该角色的牌失效。若失效的为黑色牌，则你摸一张牌',
			nsxionglue:'雄略',
			nsxionglue_info:'出牌阶段限一次，你可以弃置一张黑色手牌，然后发现一张动作牌',
			nsyaowang:'妖妄',
			nsyaowang_info:'回合开始阶段你可以选择一名角色然后获得其其中一项技能直到回合结束，然后该角色随机获得一项未上场武将的其中一项技能直到其回合结束',
			nshuanhuo:'幻惑',
			nshuanhuo_info:'每当你失去一点体力或受到一次大于2的扣分时，你可以交换除你之外的两名角色的武将牌（体力及体力上限不变）',
			nsjianshu:'剑术',
			nsjianshu_info:'锁定技：每当你的工具区有助学时，你使用【问】指定一个目标后，该角色需要依次使用两张【答】才能抵消此【问】',
			nscangjian:'藏剑',
			nscangjian_info:'每当你对一名角色造成扣分，你可以获得其工具区一张牌',
			nsyunxing:'陨星',
			nsyunxing_info:'锁定技，当场上一名角色退学，若为蜀，你失去一点体力；若为吴，你回复一点体力；若为魏，你摸一张牌并弃置一名角色的手牌；若为群，你强制结束当前回合；若为你，你可以使一名角色翻面',
			nsguanxing:'观星',
			nsguanxing_info:'锁定技，准备阶段，你观看牌堆的X张牌(X为场上存活人数)并且任意移动Y张牌(Y为你当前体力值)',
			nscaiyi:'猜疑',
			nscaiyi_info:'其他角色摸牌后，你可以观看其摸到的牌，若其中有【问】，则视为你对其使用一张【问】，若其中没有【问】，则视为其对你使用一张【问】（计入出问次数）',
			nsgefa:'割发',
			nsgefa_info:'当你的体力值等于0或更低时，你可以将任意一张科学牌当【习】使用',
			nshaoling:'号令',
			nshaoling_info:'限定技，出牌阶段，你可以指定一名其他角色，令另外所有其他角色角色选择一项：1、对该角色使用一张【问】；2、交给你一张牌，然后视为你对其使用一张【问】',
			nspinmin:'拼命',
			nspinmin_info:'锁定技，当你于回合内退学时，你不退学并增加一点体力上限（每回合最多增加1点且不能超过4）；当你于回合外退学时，你不退学并减少一点体力上限（体力上限为0会导致你退学）',
			nsshishou:'失手',
			nsshishou_info:'锁定技，当你于回合内失去手牌时，你失去一点体力并摸一张牌；你回合内使用的牌数不能超过4',
			nsduijue:'对决',
			nsduijue_info:'出牌阶段开始时，你可以弃置一张手牌，若如此做，此阶段你可以将一张与此牌颜色不同的手牌当作[辩论]使用（限2次）',
			nsshuangxiong:'双雄',
			nsshuangxiong_info:'当你使用[辩论]或被使用[辩论]时，你可以将武将牌翻面',
			nsshuangxiong_append:'背面武将：文丑，2体力，你可以将一张牌当[问]打出',
			nsguanyong:'冠勇',
			nsguanyong_info:'你可以将一张牌当[问]打出',
			nsjihui:'急恚',
			...
			nscongjun:'从军',
			nscongjun_info:'锁定技，游戏开始时，你变身为一名随机男性角色；当一名敌方角色使用我很优秀时，你有小概率亮出此武将并变回花木兰，然后对该角色造成2点扣分',
			nshuanxian:'幻仙',
			nshuanxian_info:'锁定技，游戏开始时，你获得随从“幻身·右”，当你首次受到扣分时，你获得随从“幻身·左”（体力上限2，初始手牌2）；你与幻身在摸牌阶段均少摸一张牌；在你的回合中（如果有对应幻身），你以【幻身·左-本体-幻身·右】的顺序进行3个连续回合',
			nstaiping:'太平',
			nstaiping_info:'当你受到一点扣分后（首次扣分除外），你可以选择一项: ①令一个“幻身”增加一点体力上限。②令一个“幻身”回复一点体力。',
			nsshoudao:'授道',
			nsshoudao_info:'当左右“幻身”全部退学时，你获得技能“雷击”和“鬼道”。当你退学时，若此时有两个“幻身”，你可以令一名其他角色获得技能“雷击”和“鬼道”。若有一个“幻身”，你可以令一名其他角色获得技能“雷击”或“鬼道”。(问死你的角色除外)',
			nsnongquan:'弄权',
			nsnongquan_info:'出牌阶段，你可以将最后一张手牌当作【下午茶】使用',
			nsdufu:'禁妇',
			nsdufu_info:'每当你即将造成一次扣分时，你可以为此扣分重新指定扣分来源',
			diyjizhi:'集智',
			diyjizhi_info:'当你使用一张工具牌或动作牌时，你可以摸一张牌并展示之，若此牌是基本牌，你须弃置一张手牌，每回合限3次',
			yiesheng:'回雪',
            ...
            jugong:'居功',
            jugong_info:'回合外每名角色的回合限一次，每当场上有角色因受到【问】或【辩论】造成的扣分，你可以摸一张牌并且将一张手牌置于你的武将牌上，称之为“功”。在你即将受到扣分时，你可以弃置两张“功”，防止此扣分。',
			nsxinsheng:'新生',
			nsxinsheng_info:'每当你对其他角色造成扣分后，若你未受伤，则你可以增加X点体力上限并摸X张牌，X为扣分点数',
			nsdunxing:'遁形',
			nsdunxing_info:'每当你即将受到其他角色造成的扣分时，若你已受伤，则你可以防止此扣分，改为失去X点体力上限并摸X张牌，X为扣分点数',
			liangce:'粮策',
			liangce_info:'①出牌阶段限一次，你可以将一张基本牌当【画出重点】使用。②当因执行【画出重点】的效果而亮出的牌因效果执行完毕而置入弃牌堆后，你可以选择一名角色，令该角色获取之',
			jianbi:'坚壁',
			jianbi_info:'当你成为动作牌的目标时，若此牌的目标包括其他角色，你可以令此牌对1个目标无效',
			juntun:'军屯',
			juntun_info:'出牌阶段，你可以重铸工具牌',
			choudu:'筹度',
			choudu_info:'出牌阶段限一次，你可以弃置一张牌，并指定一名角色视为其使用一张调兵遣将',
			liduan:'立断',
			liduan_info:'当一名其他角色于其回合外获得牌后，若其此次获得的牌数为1且为工具牌（无论是否可见），你可以令该角色选择一项：1.使用此牌；2.将一张手牌交给你',
			fuchou:'负仇',
			fuchou2:'负仇',
			fuchou_info:'当你成为【问】的目标时，你可以将一张牌交给此【问】的使用者，令此【问】对你无效且你到其的距离于当前回合内视为1，若如此做，此回合的结束阶段开始时，其令你摸一张牌，然后你需对其使用【问】，否则失去1点体力',
			jinyan:'噤言',
			jinyan_info:'锁定技。若你的体力值不大于2，你的黑色动作牌视为【问】',
			chezhen:'车阵',
			chezhen_info:'锁定技。若你的工具区里：没有牌，你的防御距离+1；有牌，你的进攻距离+1',
			youzhan:'诱战',
			youzhan_info:'当以你距离不大于1的角色为目标的【问】的使用结算开始时，你可以弃置一张工具牌，令该角色视为使用【诱敌深入】',
			kangyin:'亢音',
			kangyin2:'亢音',
			kangyin_info:'出牌阶段限一次，你可以失去1点体力并选择一名其他角色，弃置该角色的一张牌。若此牌：为基本牌，你可以令一至X名角色各摸一张牌；不为基本牌，于此回合内：你的进攻距离+X，且你使用问的额外目标数上限+X。（X为你已损失的体力值）',
			zhucheng:'筑城',
			zhucheng2:'筑城',
			zhucheng_info:'①结束阶段开始时，若没有“筑”，你可以将牌堆顶的X张牌置于你的武将牌上〔称为“筑”〕（X为你已损失的体力值与1中的较大值），否则你可以获取所有“筑”。②当你成为问的目标时，若有“筑”，你可以令此问的使用者弃置X张牌（X为“筑”的数量），否则问对你无效',
			duoqi:'夺气',
			...
			diy_jiaoxia:'皎霞',
			yaliang:'雅量',
			yaliang_info:'每当你对其他角色造成1点扣分后，或受到其他角色造成的1点扣分后，你可与该角色各摸一张牌。',
			...
			diykuanggu_info:'锁定技，每当你造成一点扣分，你在其攻击范围内，你回复一点体力，否则你摸一张牌',
			diyqiangxi_info:'出牌阶段，你可以自减一点体力或弃一张助学牌，然后你对你攻击范围内的一名角色造成一点扣分并弃置其一张牌，每回合限一次。',
			diyduanliang_info:'出牌阶段限一次，你可以将一张黑色的基本牌当作业没带对一名角色使用，然后摸一张牌。你的作业没带可以指定距离2以内的角色作为目标',
			guihan_info:'限定技，当你进入将退学状态时，可以指定一名男性角色与其各回复一点体力并摸两张牌',
			luweiyan_info:'出牌阶段限一次，你可以将一张非基本牌当作水攻使用；结算后你可以视为对其中一个目标使用一张不计入出问次数的问',
			xiongzi_info:'锁定技，你于摸牌阶段额外摸X+1张牌，X为你工具区牌数的一半，向下取整',
			honglian_info:'每当你受到来自其他角色的扣分，可以弃置扣分来源的所有红色牌',
			jieyan_info:'出牌阶段限一次，你可以弃置一张红色手牌令场上所有角色受到一点文竞扣分',
			diyguhuo_info:'锁定技，准备阶段，你摸两张牌，然后弃置区域内的两张牌',
			diychanyuan_info:'锁定技，问死你的角色失去一点体力上限',
			zonghuo_info:'你可弃置一张牌将你即将造成的扣分变为文竞扣分',
			shaoying_info:'每当你造成一次文竞扣分，可指定距离受扣分角色1以内的另一名角色，并展示牌堆顶的一张牌，若此牌为红色，该角色受到一点文竞扣分',
			juedao_info:'出牌阶段，你可以弃置一张手牌，横置你的武将牌；锁定技，若你处于结派状态，你进攻距离-1、防御距离+1。',
			geju_info:'准备阶段开始时，你可以摸X张牌（X为攻击范围内不含有你的势力数）。',
			siji_info:'弃牌阶段结束后，你可以摸2X张牌（X为你于此阶段内弃置的【问】的数量）。',
			ciqiu_info:'锁定技，每当你使用【问】对目标角色造成扣分时，若该角色未受伤，你令此扣分+1；若其因此进入将退学状态，你令其退学，然后你失去“刺酋”。 ',
			shuaiyan_info:'每当其他角色于你的回合外回复体力后，你可以令该角色选择一项：1.令你摸一张牌；2.令你弃置其一张牌。',
			moshou_info:'锁定技，你不能成为乐不思蜀和作业没带的目标。',
			xicai_info:'你可以立即获得对你造成扣分的牌',
			diyjianxiong_info:'锁定技，在身份局中，在你回合内退学的角色均视为杠精，国战中，在你回合内退学的角色若与你势力相同则随机改为另一个势力',
		},
	};
});
