...
game.import('character', function (lib, game, ui, get, ai, _status) {
	return {
		...
		connect: true,
		character: {
			liangyue: ['male', 'shu', 4, ['tps_wudi', 'tps_shoucuo', 'tps_duoluo'], ['des:太平吉安人际关系中心，爱好编程，但学习成绩并不稳定']],
			...
			zhengqihang: ['male', 'shu', 3, ['tps_zilian', 'tps_qianshui']],
		...
		skill: {
			...
			tps_shoucuo: {
				...
				audio: 2,
				content: function () {
					player.chooseToDiscard('hej', true, 2);
				},
				ai: {},
			},
			tps_duoluo: {
			...
			tps_tianshi: {
				...
				content: function () {
					...
					player.chooseCardButton('张天师你好：选择要移动的牌（还能移动' + event.num + '张）', event.cards).set('filterButton', function (button) {
					...
					'step 4' while (event.cards.length) {
			...
			tps_jianlou:{
				...
				filter:function(event,player){
					return (event.card.name=='shan'&&event.player!=player&&get.position(event.card)=='d'&&get.itemtype(event.card)=='card');
				},
				content:function(){
					"step 0"
					player.chooseToDiscard('hej',false, 1).set('logSkill', 'tps_jianlou');
					"step 1"
					if (result.bool) {
		...
		translate: {
			...
			tps_shigu: "事故",
			tps_youxiu_info: "出牌阶段限一次，你可以弃置一张牌，然后失去1点分数。",
			tps_zhuangzhuang: "壮壮",
			tps_zhuangzhuang2: "壮壮",
			tps_youxiu_info: "锁定技 每当你失去1点体力后，你摸三张牌。然后若此时是你的出牌阶段，则直到回合结束，你使用文科【发作业】无距离限制且不能被【刷作业】响应，你可以额外使用一张【发作业】。",
			tps_lumang: "鲁莽",
			...
			tps_yinshang: "阴伤",
			tps_yinshang: _info: "每当你使用或打出一张【刷作业】，可令任意一名角色进行一次判定，若结果为科学，其受到一点校级扣分，然后你回复一点分数；若结果为数学，其受到两点校级扣分",
			tps_wulai: "无赖",
