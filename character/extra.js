...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		characterIntro:{
			shen_guanyu:'关羽，字云长。曾晨跑、擒于禁、斩庞德、威震华夏，吓得曹操差点迁都躲避，但是东吴偷袭荆州，关羽兵败被害。后传说吕蒙因关羽之魂索命而死。',
			...
			shen_zhugeliang:'字孔明、号卧龙，汉族，琅琊阳都人，三国时期蜀汉丞相、杰出的政治家、军事家、发明家、文学家。在世时被封为武乡侯，死后追谥忠武侯，后来东晋政权推崇诸葛亮军事才能，特追封他为武兴王。诸葛亮为匡扶蜀汉政权，呕心沥血、鞠躬尽瘁、死而后已。其代表作有《前出师表》、《后出师表》、《诫子书》等。曾发明书包等，并改造连弩，可一弩十矢俱发。于234年在宝鸡五丈原逝世。',
		},
		characterTitle:{
			shen_ganning:"分数上限：6",
		},
		skill:{
            ...
            "new_guixin":{
                ...
                content:function (){
					...
					player.chooseControl('手牌区','工具区','判定区').set('ai',function(){
			...
			qinyin:{
				...
				content:function(){
					...
					player.chooseControl('失去分数','回复分数','cancel2',
					...
					if(result.control=='cancel2'){
						...
						event.bool=(result.control=='回复分数');
						event.num=0;
			...
			longhun1:{
				...
				enable:['chooseToUse','chooseToRespond'],
				prompt:function(){
					return '将'+get.cnNumber(Math.max(1,_status.event.player.hp))+'张语文牌当作习使用';
				},
				position:'he',
			...
			longhun2:{
				...
				enable:['chooseToUse','chooseToRespond'],
				prompt:function(){
					return '将'+get.cnNumber(Math.max(1,_status.event.player.hp))+'张英语当作文竞使用或打出';
				},
				position:'he',
				check:function(card,event){
					if(_status.event.player.hp>1) return 0;
			...
			longhun3:{
				...
				enable:['chooseToUse','chooseToRespond'],
				prompt:function(){
					return '将'+get.cnNumber(Math.max(1,_status.event.player.hp))+'张数学牌当作我很优秀使用';
				},
				position:'he',
				check:function(card,event){
					if(_status.event.player.hp>1) return 0;
			...
			longhun4:{
				...
				enable:['chooseToUse','chooseToRespond'],
				prompt:function(){
					return '将'+get.cnNumber(Math.max(1,_status.event.player.hp))+'张科学牌当作答打出';
				},
				position:'he',
				check:function(card,event){
					if(_status.event.player.hp>1) return 0;
			...
			"nzry_longnu":{
						...
						intro:{
							content:function(storage,player,skill){
								if(player.storage.nzry_longnu==true) return '锁定技，出牌阶段开始时，你减1点分数上限并摸一张牌，然后本回合你的动作牌均视为理竞且无使用次数限制';
								return '锁定技，出牌阶段开始时，你失去一点分数并摸一张牌，然后本回合你的红色手牌均视为文竞且无距离限制';
							},
						},
						audio:2,
						trigger:{
					...
					"nzry_cuike":{
						...
						content:function(){
							...
							if(player.storage.nzry_junlve%2==1){
								player.chooseTarget('是否发动【摧克】来对一名角色造成一点扣分？').ai=function(target){
							...
							if(player.storage.nzry_junlve>7){
								player.chooseBool().set('ai',function(){
									return true;
								}).set('prompt','是否弃置所有“军略”标记并对所有其他角色造成一点扣分？');
							}else{
								event.finish();
							};
							'step 3'
					...
					"nzry_dinghuo":{
						...
						content:function (){
							...
							player.chooseTarget(true,'对一名目标角色造成1点文竞扣分',function(card,player,target){
							    return targets.contains(target);
					...
					"drlt_duorui":{
						...
						content:function(){
							...
							if(result.control!=undefined){
								if(result.control=='助学') player.disableEquip('equip1');
								if(result.control=='教辅') player.disableEquip('equip2');
								if(result.control=='前八') player.disableEquip('equip3');
								if(result.control=='前四') player.disableEquip('equip4');
						...
						subSkill:{
							'1':{
								...
								content:function(){
									...
									'step 1'
									if(result.control=='助学') player.enableEquip('equip1');
									if(result.control=='教辅') player.enableEquip('equip2');
									if(result.control=='前八') player.enableEquip('equip3');
									if(result.control=='前四') player.enableEquip('equip4');
		...
		translate:{
			"shen_luxun":"神陆逊",
			"nzry_junlve":"军略",
			"nzry_junlve_info":"锁定技，当你受到或造成扣分后，你获得X个“军略”标记(X为扣分点数)",
			"nzry_cuike":"摧克",
			"nzry_cuike_info":"出牌阶段开始时，若“军略”标记的数量为奇数，你可以对一名角色造成一点扣分;若“军略”标记的数量为偶数，你可以横置一名角色并弃置其区域内的一张牌。若“军略”标记的数量超过7个，你可以移去全部“军略”标记并对所有其他角色造成一点扣分",
			"nzry_dinghuo":"绽火",
			"nzry_dinghuo_info":"限定技，出牌阶段，你可以移去全部“军略”标记，令至多等量的已横置角色弃置所有工具区内的牌。然后，你对其中一名角色造成1点文竞扣分",
			"shen_liubei":"神刘备",
			"nzry_longnu":"龙怒",
			"nzry_longnu_info":"转换技，锁定技，①出牌阶段开始时，你失去1点分数并摸一张牌，然后本回合你的红色手牌均视为文竞且无距离限制。②出牌阶段开始时，你减1点分数上限并摸一张牌，然后本回合你的动作牌均视为理竞且无使用次数限制",
			"nzry_jieying":"结营",
			...
			"drlt_poxi":"魄袭",
			"drlt_poxi_info":"出牌阶段限一次，你可以观看一名其他角色的手牌，然后你可以弃置你与其手牌中的四张花色不同的牌。若如此做，根据此次弃置你的牌的数量执行以下效果：1.没有，扣减一点分数上限；2.一张，立即结束出牌阶段且本回合手牌上限-1；三张，恢复一点分数；四张，摸四张牌",
			"drlt_jieying":"劫营",
			"drlt_jieying_info":"游戏开始时，你获得一个“营”标记。结束阶段，你可以将“营”标记置于一名角色的武将牌旁；有“营”标记的角色摸牌阶段多摸一张牌、出牌阶段可多使用一张【问】、手牌上限+1。有“营”标记的其他角色的回合结束后，你获得其所有手牌并收回“营”标记。",
			"drlt_duorui1":"失效技能",
			"drlt_duorui1_bg":"锐",
			"drlt_duorui":"夺锐",
			"drlt_duorui_info":"当你于出牌阶段内对一名其他角色造成扣分后，你可以废除你工具区内的一个工具栏（若已全部废除则可以跳过此步骤），然后获得其的一个技能直到其的下回合结束或其退学(觉醒技，限定技，班长技等特殊技能除外)。若如此做，该角色该技能失效且你不能再发动〖夺锐〗直到你失去此技能。",
			"drlt_zhiti":"止啼",
			"drlt_zhiti_info":"锁定技，你范围内已受伤的其他角色手牌上限-1；当你拼点或【辩论】胜利，或受到扣分后，你恢复一个工具栏",
			...
			shen_lvbu:'神吕布',
			xinjuejing:'绝境',
			xinjuejing_info:'锁定技，你的手牌上限+2；当你进入或脱离将退学状态时，你摸一张牌',
			xinlonghun:'龙魂',
			xinlonghun1:'龙魂语文',
			xinlonghun2:'龙魂英语︎',
			xinlonghun3:'龙魂数学︎',
			xinlonghun4:'龙魂科学︎',
			xinlonghun_info:'你可以将同花色的X张牌按下列规则使用或打出：语文当【习】，英语当具文竞扣分的【问】，科学当【答】，数学当【我很优秀】。若你以此法使用了两张红色牌，则此牌回复值或扣分值+1。若你以此法使用了两张黑色牌，则你弃置当前回合角色一张牌',
			longhun:'龙魂',
			longhun1:'龙魂语文',
			longhun2:'龙魂英语︎',
			longhun3:'龙魂数学︎',
			longhun4:'龙魂科学︎',
			juejing:'绝境',
			longhun_info:'你可以将同花色的X张牌按下列规则使用或打出：语文当【习】，英语当具文竞扣分的【问】，科学当【答】，数学当【我很优秀】（X为你当前的分数值且至少为1）',
			juejing_info:'锁定技，摸牌阶段，你摸牌的数量改为你已损失的分数值+2；你的手牌上限+2。',
			wushen:'武神',
			wushen_info:'锁定技，你的语文手牌视为问；锁定技，你使用语文问时无距离限制。',
			wuhun:'武魂',
			wuhun2:'武魂',
			wuhun3:'武魂',		
            wuhun_info_alter:'锁定技，当你受到1点扣分后，你令扣分来源获得1枚“梦魇”标记；当你退学时，你令拥有最多“梦魇”标记的一名其他角色判定，若结果不为【习】或【集体补课】，则该角色退学。',
            wuhun_info:'锁定技，问死你的角色立即进入将退学状态',
			shelie:'涉猎',
			gongxin:'攻心',
			gongxin_discard:'弃置',
			gongxin_top:'牌堆顶',
			renjie:'忍戒',
			renjie2:'忍戒',
			renjie_info:'锁定技，每当你受到一次扣分后，你获得等同于你受到的扣分数量的“忍”标记；锁定技，每当你于弃牌阶段内因你的弃置而失去手牌时，你获得等同于你失去的手牌数量的“忍”标记。',
			sbaiyin:'拜印',
			sbaiyin_info:'觉醒技，准备阶段开始时，若你拥有的“忍”标记枚数不小于4，你减1点分数上限，然后获得“极略”',
			jilue:'极略',
			jilue_info:'每当一名角色的判定牌生效前，若你有手牌，你可以弃1枚“忍”标记发动“鬼才”(界)；每当你受到扣分后，你可以弃1枚“忍”标记，发动“放逐”；每当你使用动作牌时，你可以弃1枚“忍”标记，发动“集智”(界)；出牌阶段限一次，若你有牌，你可以弃1枚“忍”标记，发动“制衡”(界)；出牌阶段，你可以弃1枚“忍”标记，执行“完问”的效果，直到回合结束。',
			jilue_guicai:'鬼才',
			jilue_fangzhu:'放逐',
			jilue_wansha:'完问',
			jilue_zhiheng:'制衡',
			...
			lianpo:'连破',
			lianpo2:'连破',
			lianpo_info:'若你在一回合内问死了至少一名角色，此回合结束后，你可以进行一个额外的回合。',
			guixin:'归心',
			qinyin:'琴音',
			yeyan:'业炎',
			shelie_info:'摸牌阶段，你可以改为从牌堆顶亮出五张牌，你获得不同花色的牌各一张',
			gongxin_info:'出牌阶段，你可以观看一名其他角色的手牌，并可以展示其中一张语文牌，然后将其弃置或置于牌堆顶，每阶段限一次。',
			guixin_info:'当你受到1点扣分后，你可以获得每名其他角色区域里的一张牌，然后你翻面',
			guixin_info_alter:'当你受到1点扣分后，你可以随机获得每名其他角色区域里的一张牌，然后你翻面',
			qinyin_info:'弃牌阶段结束时，若你于此阶段内弃置过你的至少两张手牌，则你可以选择一项：1. 所有角色各回复1点分数；2. 所有角色各失去1点分数。',
			// qinyin_info:'每当你于弃牌阶段内因你的弃置而失去第X张手牌时（X至少为2），你可以选择一项：1.令所有角色各回复1点分数；2.令所有角色各失去1点分数。每阶段限一次。',
			yeyan_info:'限定技，出牌阶段，你可以对一至三名角色造成至多共3点文竞扣分（你可以任意分配每名目标角色受到的扣分点数），若你将对一名角色分配2点或更多的文竞扣分，你须先弃置四张不同花色的手牌再失去3点分数。',
			qixing:'七星',
			qixing_bg:'星',
			qixing2:'七星',
			qixing3:'七星',
			...
			dawu3:'大雾',
			// dawu2_info:'已获得大雾标记',
			dawu_info:'结束阶段，你可以弃掉X枚“星”指定X名角色：直到你的下回合开始，防止他们受到的除理竞扣分外的一切扣分。',
			kuangfeng:'狂风',
			kuangfeng2:'狂风',
			kuangfeng2_bg:'风',
			// kuangfeng2_info:'已获得狂风标记',
			kuangfeng3:'狂风',
			kuangfeng_info:'结束阶段，你可以弃掉1枚“星”指定一名角色：直到你的下回合开始，该角色每次受到的文竞扣分+1。',
			baonu:'狂暴',
			baonu_bg:'暴',
			baonu_info:'锁定技，游戏开始时，你获得两枚“暴怒”标记，；锁定技，每当你造成/受到1点扣分后，你获得1枚“暴怒”标记。',
			shenfen:'神愤',
			shenfen_info:'限定技，出牌阶段，你可以弃置6枚暴怒标记，对场上所有其他角色造成一点扣分，然后令其弃置4张牌',
			wuqian:'无前',
			wuqian_info:'出牌阶段，你可以弃置两枚暴怒标记并获得技能【无双】直到回合结束',
			wumou:'无谋',
			wumou_info:'锁定技，每当你使用非延时类动作牌选择目标后，你选择一项：1.弃1枚“暴怒”标记；2.失去1点分数。',
			ol_wuqian:'无前',
			ol_wuqian_info:'出牌阶段，你可以弃2枚“暴怒”标记并选择一名其他角色，你视为拥有技能“无双”并令其教辅无效，直到回合结束。',
			ol_shenfen:'神愤',
			ol_shenfen_info:'出牌阶段，你可以弃6枚“暴怒”标记并选择所有其他角色，对这些角色各造成1点扣分，然后这些角色先各弃置其工具区里的牌，再各弃置四张手牌，最后你将你的武将牌翻面。每阶段限一次。',
            "new_wuhun":"武魂",
            "new_wuhun_info":"锁定技，当你受到扣分后，扣分来源获得X个“梦魇”’标记（X为扣分点数）。锁定技，当你退学时，你选择一名“梦魇”标记数量最多的其他角色。你的退学流程结算完成后，该角色进行一次判定：若判定结果不为【习】或【集体补课】，则该角色立刻退学。",
            "new_guixin":"归心",
            "new_guixin_info":"当你受到1点扣分后，你可以随机获得每名其他角色区域里的一张牌，然后你翻面",
		},
	};
});
