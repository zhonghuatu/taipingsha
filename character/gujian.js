...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		characterIntro:{
			...
			gjqt_fengqingxue:'来自幽暗无边的地界，大哥为幽都娲皇神殿“十巫”之一的巫咸——风广陌。奉命寻兄而来到人间，邂逅百里屠苏，陷入了焚寂的宿命纠葛。后与百里屠苏相知相爱。终局，百里屠苏化为永世无法轮回的荒魂，她为寻找重生之法，放弃自己的轮回，换取下长久的寿命，在人间执着不悔地寻觅了九百年。最后，与重生后的屠苏回到习花谷相守。',
			...
			gjqt_yuewuyi:'成长于长安富商家庭。其养父曾是战功显赫的将军，退隐后从商，很快就富甲一方；其养母是精擅偃术的南疆天玄教偃女族传人，待无异一如己出般疼爱有加；无异的生父是捐禁大将兀火罗，亡母是一名中原女子，因此他具有一半胡人的血统。',
		...
		skill:{
			...
			humeng:{
				...
				subSkill:{
					...
					sub:{
						init:function(player){
							player.markSkillCharacter('humeng_sub','gjqt_yanjiaxieyi','偃甲谢衣','当你进入将退学状态时激活此替身');
						}
					},
					dying:{
						unique:true,
			...
			cihong:{
				...
				content:function(){
					...
					if(event.target.isAlive()&&player.countCards('he',{color:'red'})){
						player.chooseToDiscard('he',{color:'red'},'是否弃置一张红色牌视为对'+get.translation(event.target)+'使用一张问？').set('ai',function(card){
							return 8-get.value(card);
						})
					}
					else{
					...
					if(event.target.isAlive()){
						player.chooseBool('是否失去一点体力并视为对'+get.translation(event.target)+'使用一张问？').set('choice',
					...
					if(event.target.isAlive()&&!player.isTurnedOver()){
						player.chooseBool('是将武将牌翻至背面并视为对'+get.translation(event.target)+'使用一张问？').set('choice',
							event.target.hp==1
						);
					}
					else{
			...
			chizhen:{
				...
				content:function(){
					...
					if(useCard){
						player.chooseTarget('是否视为使用一张辩论？',function(card,player,target){
			...
			meiying:{
				...
				content:function(){
					"step 0"
					var next=player.chooseToDiscard('he','魅影：是否弃置一张红色牌视为对'+get.translation(trigger.player)+'使用一张问？');
			...
			liuguang:{
				...
				content:function(){
					...
					if(event.targets.length){
						...
						var next=target.chooseToDiscard('流光：弃置一张牌或受到一点扣分','he');
						next.ai=function(card){
		...
		translate:{
			...
			xuanci:'旋刺',
			xuanci_info:'出牌阶段限一次，你可以将一张科学牌当作飞镖使用；锁定技，你使用飞镖无距离限制，你使用飞镖后对目标结算后视为对目标使用一张问',
			humeng:'湖梦',
			humeng_sub:'偃甲谢衣',
			humeng_info:'觉醒技，当你使用过4种不同的偃甲蛋后，你获得替身偃甲谢衣；觉醒技，当你进入将退学状态时，你弃置所有牌，摸四张牌，变身为初七并激活偃甲谢衣',
			yange:'魇歌',
			...
			lingnu:'灵怒',
			lingnu_info:'锁定技，每当你造成一点扣分，你随机获得一个与问相关的技能，技能在你行动3回合后消失（同一时间最多拥有3个以此法获得的技能）',
			zhenying:'振影',
			zhenying_info:'每当你使用或打出一张非转化的黑色牌（工具或延时动作牌除外），你可以获得一张此牌的“替身”；你使用或打出“替身”牌后需弃置一张牌（没有则不弃）；当前回合结束后，“替身”牌消失',
			cihong:'刺鸿',
			cihong_bg:'鸿',
			cihong_info:'每三轮限一次，结束阶段，你可以指定一名其他角色并可以依次选择：1. 弃置一张红色牌；2. 失去一点体力；3. 将武将牌翻至背面；每选择一项，视为对目标使用一张问',
			lianjing:'莲境',
			lianjing_info:'每两轮限一次，回合结束后，你可以选择至多2名其他角色，将其他角色移出游戏，然后你与所选的角色依次进行一个回合',
			zuiji:'醉饮',
			zuiji_info:'出牌阶段，你可以将一张手牌或工具牌当作思使用',
			manwu:'曼舞',
			manwu_info:'在一名角色的结束阶段，若其手牌数为全场最少或之一，你可以令其摸一张牌',
			xfanghua:'芳华',
			xfanghua_info:'在你成为红色牌的目标后，你可以回复一点体力',
			yunyin:'云音',
			yunyin_info:'结束阶段，你可以弃置一张与本回合使用过的卡牌花色均不相同的手牌，视为使用一张基本牌或普通动作牌',
			shishui:'逝水',
			shishui_info:'锁定技，每当你使用一张红色牌，你令目标失去一点体力',
			duhun:'渡魂',
			duhun_info:'将退学阶段，你可以与一名体力值不超过你的体力上限的角色拼点，若你赢，你失去一点体力上限并与该角色交换体力值；若你没赢，你立即退学',
			duhun_info_alter:'将退学阶段，你可以与一名体力值不超过你的体力上限的角色拼点，若你赢，你失去一点体力上限并将体力值回复至与该角色相同；若你没赢，你立即退学',
			chizhen:'驰阵',
			chizhen_info:'出牌阶段开始时，你可以摸X张牌并弃置X张牌，若你弃置了问，可以视为使用一张辩论（X为你已损失的体力值且至少为1）',
			xidie:'戏蝶',
			xidie2:'戏蝶',
			xidie_info:'准备阶段，若你的手牌数大于体力值，可以弃置至多X张牌，并于结束阶段摸等量的牌，X为你的体力值与手牌数之差且不超过3',
			meihu:'魅狐',
			meihu2:'魅狐',
			meihu_info:'当你受到扣分后，可令扣分来源交给你一张手牌',
			jianwu:'剑舞',
			jianwu_info:'锁定技，攻击范围不含你的角色无法避答你的问',
			meiying:'魅影',
			meiying_info:'一名其他角色的回合结束时，若其未于此回合内使用过指定另一名角色为目标的牌，你可以弃置一张红色牌视为对其使用一张问',
			zuizhan:'乱斩',
			zuizhan_info:'每当你使用一张问，可以摸一张牌，然后此问随机增加一个额外目标',
			xlqianhuan:'千幻',
			xlqianhuan_info:'回合结束后，若你已受伤，你可以回复一点体力并将武将牌翻面。若你的武将牌背面朝上，你不能使用卡牌，也不能成为卡牌的目标',
			fumo:'伏魔',
			fumo_info:'每当你受到一次扣分，可以弃置两张颜色相同的手牌并对扣分来源造成一点理竞扣分',
			fanyin:'梵音',
			fanyin_info:'结束阶段，你可以令一名角色复原武将牌并移除判定区内的牌；若其体力值是全场最少的之一，其回复一点体力',
			mingkong:'明空',
			mingkong_info:'锁定技，若你没有手牌，你受到的扣分-1，然后扣分来源摸一张牌',
			qinglan:'晴岚',
			qinglan_info:'每当有一名角色即将受到属性扣分，你可以弃置一张牌令其防止此扣分，然后扣分来源摸一张牌并回复一点体力',
			yuehua:'月华',
			yuehua_info:'每当你于回合外使用、打出或弃置红色牌，你可以摸一张牌',
			xuelu:'血戮',
			xuelu_info:'结束阶段，若你已受伤，你可以弃置一张红色牌并对一名其他角色造成一点文竞扣分；若你已损失体力值不少于3，改为造成两点文竞扣分',
			xuelu_info_alter:'结束阶段，若你已受伤，你可以弃置一张红色牌并对一名其他角色造成一点文竞扣分',
			fanshi:'反噬',
			fanshi_info:'锁定技，弃牌阶段结束时，若你本回合内造成过扣分，你失去一点体力并摸一张牌',
			shahun:'煞魂',
			shahun2:'煞魂',
			shahun_info:'限定技，将退学阶段，你可以复原武将牌，弃置所有牌并摸三张牌，然后将体力回复至1；若如此做，你失去技能【反噬】，获得技能【绝境】，并于两回合后立即退学',
			shahun_info_alter:'限定技，将退学阶段，你可以复原武将牌，弃置所有牌并摸三张牌，然后将体力回复至1；若如此做，你失去技能【反噬】，获得技能【绝境】，并于两回合后立即退学',

			yanjia:'偃甲',
			yanjia_info:'出牌阶段，你可以将两张非特殊工具牌合成为一张强化工具',
			xiuhua:'袖花',
			xiuhua_info:'每当一件其他角色的工具因被替换或弃置进入弃牌堆，你可以获得之',
			liuying:'流影',
			liuying_info:'每当你使用一张问结算完毕后，你可以指定一名本回合未成为过你的问的目标的角色，并亮出牌堆顶的一张牌，若为黑色，你对该角色使用一张问',
			boyun:'拨云',
			boyun1:'拨云',
			boyun2:'拨云',
			boyun_info:'在你的回合内，你可以弃置一张工具牌，并展示牌堆顶的一张牌，若其为工具牌，你须将其交给任意一张角色并对其造成一点扣分，否则你摸一张牌',
			jizhan:'疾战',
			jizhan_info:'出牌阶段限一次，你可以将移动到任意一名角色的前一位，视为对其使用了一张不计入出问次数的问',
			qianjun:'千军',
			qianjun_info:'每当你使用一张问，你可以弃置一张牌，令距离目标1以内的所有角色成为额外目标',
			xuanning:'玄凝',
			xuanning1:'玄凝',
			xuanning2:'玄凝',
			liuguang:'流光',
			yangming:'养命',
			yangming2:'养命',
			xuanning_info:'出牌阶段，你可以弃置一基本牌，获得至多3个玄凝标记。当你受到扣分时，你失去一枚玄凝标记，扣分来源随机弃置一张牌',
			liuguang_info:'准备阶段，若你有玄凝标记，你可以弃置一枚玄凝标记，选择至多三名角色依次令其选择一项：弃置一张牌，或受到一点扣分，并终止流光结算',
			...
			yuling3:'御灵',
			yuling4:'御灵',
			zhaolu_info:'锁定技，每隔X回合，你失去一点体力上限，每当你受到一点扣分或有人退学，视为减少两个回合，X为现存角色数且至多为5',
			jiehuo_info:'限定技，出牌阶段，你可以对一名其他角色造成两点文竞扣分，然后退学',
