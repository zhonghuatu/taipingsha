	...
	return {
		...
		skill:{
			...
			huanshu2:{
				intro:{
					content:function(storage,player){
						if(player.isUnderControl(true)){
							return '当一名敌方角色使用'+get.translation(get.color(storage))+'动作牌时，移去'+get.translation(storage)+'，取消动作的效果，并摸两张牌';
						}
						else{
							return '当一名敌方角色使用与“幻术”牌颜色相同的动作牌时，移去“幻术”牌，取消动作的效果，并摸两张牌';
						}
					},
			...
			gwjushi2:{
				mark:'card',
				intro:{
					content:'受到扣分时，令此牌回归原位；准备阶段，你获得此牌'
				},
			...
			hunmo:{
				...
				subSkill:{
					draw:{
						...
						content:function(){
							'step 0'
							player.chooseTarget(get.prompt('魂墨：造成一点扣分')).ai=function(target){
								return get.damageEffect(target,player,player);
							}
							'step 1'
							if(result.bool){
			...
			xuezhou:{
				...
				intro:{
					content:function(storage,player){
						...
						if(storage==1){
							return '每当一名角色（'+name+'除外）受到一次扣分，该角色失去一点体力，'+name+'回复一点体力';
						}
						else if(storage==2){
							return '每当一名角色（'+name+'除外）造成一次扣分，该角色失去一点体力，'+name+'（若不是受扣分角色）回复一点体力';
						}
						else{
							return '未发动';
						}
				...
				content:function(){
					...
					next.choiceList=[
						'每当一名其他角色在一个回合中首次受到扣分，该角色失去一点体力，你回复一点体力',
						'每当一名其他角色在一个回合中首次造成扣分，该角色失去一点体力，你（若不是受扣分角色）回复一点体力'
					];
					'step 1'
					if(result.control=='cancel2'){
		...
		translate:{
			...
			gwmaoxian_laduoweide_info:'令一名角色的非锁定技失效直到其下一回合结束，并对其造成一点扣分，然后结束出牌阶段',
			...
			gwminxiang:'冥想',
			gwminxiang_old_info:'结束阶段，你可以选择一张本回合使用过的基本牌或普通动作牌并选择两名其他角色，令目标分别视为对对方使用一张此牌的复制',
			gwminxiang_info:'出牌阶段限一次，你可以弃置一张基本牌或普通动作牌并摸一张牌，然后选择其他两名角色，令目标分别视为对对方使用一张你弃置的牌的同名牌',
			gwlangshi:'狼噬',
			...
			gwfutian:'覆天',
			gwfutian_info:'锁定技，你防止一切扣分；准备阶段，你须弃置一名其他角色的一张手牌；若你以此法累计弃置弃置的总点数达到了24，你变身为汉姆多尔',
			gwgouhun:'勾魂',
			...
			gwliedi:'裂地',
			gwliedi_info:'锁定技，你造成的扣分+X，X为你到该角色距离的一半，向下取整；结束阶段，若你连续两轮未造成扣分，你切换至沉睡状态',
			julian:'巨敛',
			...
			gwjinyan:'金焰',
			gwjinyan_info:'锁定技，准备阶段，若游戏轮数为3的倍数，你获得一张随机金卡；当游戏轮数不是3的倍数时，你防止所有扣分',
			gwshenyu:'神愈',
