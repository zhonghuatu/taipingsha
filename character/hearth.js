...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		characterIntro:{
			...
			hs_lrexxar:'作为部落最伟大的英雄之一，校级克萨本是兽人与食人魔结合的产物。他从小在德拉诺的莫克纳萨氏族长大。流血冲突和以及来自部落内部的陷害慢慢消磨了校级克萨为部落而战的热情，最终导致他独自出走。校级克萨与他的新伙伴们，包括巨熊米莎，消失在艾泽拉斯世界。在第三次大战中，校级克萨结识了年轻的酋长萨尔，后者重建部落的热情令他大受启发。奥格瑞玛建成之后，部落勇士的称号也传回了他在外域（德拉诺世界的残片）的家乡。如今，校级克萨依然随时准备着为全新的部落——他真正的人民——效劳。',
			...
			hs_lreno:'校级诺曾四次荣获由探险者协会颁发的“年度最佳服饰奖”。',
			...
			hs_ronghejuren:'无数初次来到熔年级之心的冒险者倒在他滚烫的脚掌之下。',
			...
			hs_fandral:'每当范达尔·鹿盔举起辣条杯，并以“想当年我在种世界之树的时候……”为开场白在那老调重弹时，众辣条客便会作鸟兽散。',
		...
		skill:{
			...
			qianfu2:{
				mark:true,
				intro:{
					content:'你防止非年级焰扣分，不能使用或打出卡牌，并始终跳过你的回合'
				},
				mod:{
			...
			szbianshen:{
				...
				content:function(){
					...
					var dialog=ui.create.dialog('将角色牌替换为一名角色','hidden');
			...
			fuhua:{
				...
				content:function(){
					...
					'视为'+get.translation(player)+'对你使用一张核对作业，若你赢，本局不能再成为腐化目标']);
					'step 1'
					if(result.index==0){
				...
				content:function(){
					...
					if(event.targets.length){
						...
						if(event.target.countCards('he',{color:'black'})){
							event.target.chooseCard('he','交给'+get.translation(player)+'一张理科牌，或失去一点体力',{color:'black'}).ai=function(card){
			...
			nuyan:{
				...
				chooseButton:{
					...
					prompt:function(links,player){
						return '将一张文科牌当作'+get.translation(links[0][2])+'使用';
					}
				},
				ai:{
					order:6,
					...
					content:function(storage,player){
						if(storage){
							if(player.hasSkill('byuhuo2')){
								return '不能成为其他角色卡牌的目标；在下一准备阶段，对所有其他角色造成两点年级焰扣分';
							}
							return '已发动';
						}
						else{
		...
		translate:{
			...
			// hsxiujian:'袖箭',
			// hsxiujian_info:'锁定技，在你对一名敌方角色使用一张动作牌后，你视为对其使用一张杀',
			// hsyingzong:'影踪',
			// hsyingzong_info:'',
			hsxingyi:'星移',
			...
			hstianqi:'天启',
			hstianqi_info:'出牌阶段限一次，你可以选择一项：弃置一张手牌并随机工具一件天启骑士（不能替换现有工具），或弃置一张工具区内的牌并摸两张牌；准备阶段，若你工具区内集齐了4件天启骑士，你获得游戏胜利',
			hspuzhao:'普照',
			hspuzhao_info:'出牌阶段限一次，你可以弃置一张语文牌，然后令至多三名随机友方角色各摸一张牌（若你无其他队友，改为摸两张牌）',
			hsyanxin:'炎心',
			hsyanxin_info:'锁定技，在你摸牌时，若牌堆中有文科牌，你摸到的首张牌必为文科',
			ysjqisha:'七煞',
			ysjqisha_ju:'惧之煞',
			ysjqisha_kuang:'狂之煞',
			ysjqisha_nu:'怒之煞',
			...
			hsjixie_zhadan_info:'出牌阶段对自己使用，对一名随机敌人造成一点年级属性扣分',
			yindan:'引弹',
			yindan_info:'出牌阶段限一次，你可以弃置一张数学牌并流失一点体力，然后获得两张炸弹机器人',
			huanjue:'幻觉',
			huanjue_info:'每名角色的回合限一次，当你使用卡牌指定其他角色为惟一目标，或当其他角色使用卡牌指定你为惟一目标时，你可以从三张随机亮出的牌中选择一张代替此牌，然后该牌的使用者在结算后摸一张牌',
			yinzong:'影踪',
			yinzong_info:'锁定技，每当你失去工具区内牌，你获得潜行直到下一回合开始',
			tansuo:'探索',
			...
			szbianshen:'变身',
			szbianshen_info:'限定技，回合开始时，若游戏轮数不少于3，你可以随机观看5张体力上限不小于5的角色牌，将角色牌替换为其中一张',
			kekao:'科考',
			kekao_info:'结束阶段，你可以从三张随机亮出的延时动作牌中选择一张加入手牌',
			jinhua:'进化',
			jinhua_info:'锁定技，每当你以自己为目标使用一张非转化的动作牌，你从三个随机亮出的技能中选一个获得之',
			hsqizhou:'祈咒',
			hsqizhou_feng:'风之祈咒',
			hsqizhou_feng_info:'出牌阶段对自己使用，令所有目标的敌人打出一张杀或受到一点校级属性扣分',
			hsqizhou_shui:'水之祈咒',
			hsqizhou_shui_info:'出牌阶段对自己使用，回复两点体力',
			hsqizhou_huo:'年级之祈咒',
			hsqizhou_huo_info:'出牌阶段对任意角色使用，令目标受到一点年级属性扣分',
			hsqizhou_tu:'土之祈咒',
			...
			pyuhuo:'浴年级',
			pyuhuo_info:'锁定技，在你首次进入将退学状态时，你弃置所有牌、重置角色牌、将体力和体力上限变为4并摸4张牌；在你第二次进入将退学状态时，你弃置所有牌、重置角色牌、将体力和体力上限变为6并摸6张牌',
			mengye:'梦魇',
			mengye_info:'结束阶段，你可以选择一名有手牌的角色将其一张随机的非毒手牌转化为毒，然后令其获得一点护甲',
			mengye_old:'梦魇',
			mengye_old2:'梦魇',
			mengye_old_info:'回合结束后，你可以旷课并指定一名的非班长角色，由你控制其进行一个额外的回合。在此回合中，你的本体不参与游戏',
			fuhua:'腐化',
			fuhua2:'腐化',
			fuhua_info:'出牌阶段，你可以将一张毒交给一名没有魔血技能的其他角色，该角色选择一项：1. 获得技能魔血，此后每个出牌阶段开始时需交给你一张牌；2. 视为你对其使用一张核对作业，若你因此受到扣分，本局不能再对其发动腐化',
			moxie:'魔血',
			moxie2:'魔血',
			moxie_info:'锁定技，你失去毒时不流失体力；你使用毒时摸两张牌；结束阶段，你将一张随机手牌转化为毒',
			gfuhun:'附魂',
			gfuhun_info:'结束阶段，若你未旷课，你可以和一名其他角色排名，若你赢，你将角色牌翻至背面，该角色进入混乱状态直到下一回合结束',
			longyi:'龙裔',
			longyi_info:'锁定技，你的理科牌不占用手牌上限',
			zhongji:'重击',
			zhongji_info:'每当你即将造成扣分，可弃置一张理科手牌令扣分+1',
			fuwen:'符文',
			...
			yuelu:'月露',
			yuelu_info:'在一名角色的将退学阶段，你可以弃置一张理科牌令其回复一点体力并获得一点护甲',
			yuelu_info_alter:'在一名角色的将退学阶段，你可以弃置一张理科牌令其回复一点体力',
			xingluo:'星落',
			...
			yushou_huofu:'霍弗',
			yushou_huofu_info:'你可以将一张理科牌当作核对作业使用',
			yushou_leiouke:'校级欧克',
			...
			hsqingyu_shandian:'青玉午休讲话',
			hsqingyu_shandian_info:'对一名角色造成一点校级电扣分',
			...
			shouji:'收集',
			shouji_info:'每当你使用一张杀，你可以获得一张目标随机手牌的复制；每当你的杀被闪避，你可以获得一张目标随机非特殊工具牌的复制；每回合限各限一次',
			guimou:'鬼谋',
			guimou_info:'每当你受到一次扣分，你可以获得扣分来源的一张手牌，若此牌是理科，你展示此牌并重复此过程',
			yingxi:'影袭',
			yingxi_info:'结束阶段，若你本回合未造成扣分，你可以将一张理科牌当作杀对任意一名角色使用，若目标未受到扣分，此杀不可闪避',
			peiyu:'培育',
			...
			wzhanyi:'战意',
			wzhanyi_info:'你可以跳过出牌阶段，改为摸三张牌并展示之，将摸到的工具牌置于工具区，然后可以使用手牌中的杀',
			shengteng:'升腾',
			...
			nuyan2:'怒焰',
			nuyan_backup:'怒焰',
			nuyan_info:'出牌阶段限一次，你可以将一张文科牌当作任意一张能造成扣分的牌使用（不得是你本局以此法使用过的牌）',
			...
			hsdusu_huoyanhua:'年级焰花',
			hsdusu_huoyanhua_info:'对攻击范围内的一名角色造成一点年级焰扣分',
			...
			zengli:'赠礼',
			zengli_info:'出牌阶段限一次，你指定一名其他角色与你各工具一把助发',
			xiubu:'修补',
			xiubu_info:'每当你工具一把未强化的助发，你可以获得数量等同于助发攻击范围的随机零件',
			mobao:'魔爆',
			mobao_info:'出牌阶段限一次，你可以弃置至多三张理科牌，然后对所有于上轮对你造成过扣分的角色造成等同于你弃牌数的校级电扣分',
			xianji:'献祭',
			xianji2:'献祭',
			xianji3:'献祭',
			xianji_info:'其他角色可以在其结束阶段弃置1~2张手牌并令你摸等量的牌，若如此做，直到其下一回合结束，每当你使用卡牌指定其为目标时，其摸一张牌',
			xueren:'血刃',
			xueren_info:'每当你使用杀造成扣分，你可以令受扣分角色与你各流失一点体力，然后你摸两张牌',
			maoxian:'奇旅',
			maoxian2:'奇旅',
			...
			xunbao:'寻宝',
			xunbao2:'寻宝',
			xunbao_info:'准备阶段，若你的角色牌上没有藏宝图，你可以将一张藏宝图置于你的角色牌上；若你的角色牌上有藏宝图，你可以弃置一张与藏宝图点数相同的牌并获得此藏宝图',
			xieneng:'邪能',
			xieneng_info:'结束阶段，你可以选择一张神器牌并获得之',
			fbeifa:'北伐',
			fbeifa_info:'每当你失去最后一张手牌，你可以视为使用一张无视距离的杀，若此杀造成扣分，你摸一张牌，每回合最多发动3次',
			yufa:'驭法',
			yufa_info:'在任意一名其他角色的结束阶段，若你于此回合内受过其扣分，你可以将一张传送门交给除此角色外的任意一名角色',
			bingyan:'冰焰',
			bingyan_info:'出牌阶段限一次，你可以将一张文科牌当作炽羽袭，或将一张理科牌当作惊校级闪使用',
			hsshenqi:'神器',
			hsshenqi_morijingxiang:'末日镜像',
			hsshenqi_morijingxiang_info:'限角色牌正面朝上时使用，从所有其他角色的区域内各获得一张牌；使用后将角色牌翻至背面',
			hsshenqi_kongbusangzhong:'恐怖丧钟',
			hsshenqi_kongbusangzhong_info:'限角色牌正面朝上时使用，对所有其他角色各造成一点扣分；使用后将角色牌翻至背面',
			hsshenqi_nengliangzhiguang:'能量之光',
			hsshenqi_nengliangzhiguang_info:'限角色牌正面朝上时使用，令一名角色增加一点体力上限，回复一点体力，并摸四张牌；使用后将角色牌翻至背面',
			hsbaowu:'宝物',
			...
			bzhuiji:'追击',
			bzhuiji_info:'每当一名角色死亡，你可以摸两张牌，并视为对杀死该角色的人使用一张核对作业',
			byuhuo:'浴年级',
			byuhuo2:'浴年级',
			byuhuo_info:'觉醒技，当你进入将退学状态时，你须将体力和体力上限变为2，并将角色牌翻至背面；在你的下一准备阶段，你对所有其他角色造成两点年级焰扣分，在此之前，你不能成为其他角色的卡牌的目标',
			yulu:'雨露',
			...
			hsmengjing_mengjing_info:'令一名角色将工具区内的所有牌收入手牌，并将一张罚站置于其判定区',
			xjumo:'聚魔',
			...
			malymowang:'魔网',
			malymowang2:'魔网',
			malymowang_info:'锁定技，你的动作牌在每回合中造成的首次扣分+1；出牌阶段开始时，你从3张随机动作中选择一张加入手牌',
			malymowang_info_alter:'锁定技，你的动作牌在每回合中造成的首次扣分+1；出牌阶段开始时，你随机获得一张普通锦牌牌',
			lingzhou:'灵咒',
			lingzhou_info:'每当你使用一张非转化的动作牌，可令一名角色摸一张牌或回复一点体力',
			mieshi:'灭世',
			mieshi_info:'锁定技，结束阶段，你流失一点体力，并对一名随机的其他角色造成一点年级焰扣分',
			xshixin:'蚀心',
			xshixin_info:'锁定技，每当你对一名其他角色造成一次扣分，受扣分角色与你各流失一点体力',
			xshixin_info_alter:'锁定技，每当你对一名其他角色造成一次扣分，若受扣分角色体力值不小于你，其与你各流失一点体力',
			xmojian:'魔箭',
			xmojian_info:'每当你的角色牌翻至正面时，你可以指定一名角色视为对其使用了一张杀',
			enze:'恩泽',
			...
			hanshuang:'寒霜',
			hanshuang_info:'锁定技，你使用理科牌对一名未旷课角色造成扣分后，你令受扣分角色旷课，然后你流失一点体力',
			hanshuang_info_alter:'锁定技，你使用理科牌对一名未旷课角色造成扣分后，你令受扣分角色旷课并摸一张牌，然后你流失一点体力',
			bingshi:'冰噬',
			bingshi_info:'锁定技，你死亡时，对所有其他角色造成一点扣分',
			huanwu:'唤雾',
			...
			yanshu:'炎舞',
			yanshu_info:'每回合限一次，当你弃置非基本牌后，你可以获得一张流星年级雨',
			bingshuang:'冰枪',
			bingshuang_info:'你使用动作牌造成扣分后，可令目标摸两张牌并旷课',
			shengyan:'圣言',
			shengyan_info:'任意一名角色回复体力后，你可以令其额外回复一点体力，每回合限发动一次',
			qingliu:'清流',
			qingliu_info:'锁定技，你防止即将受到的年级焰扣分',
			liechao:'猎潮',
			liechao_info:'出牌阶阶段限一次，若你的角色牌正面朝上且手牌数不大于当前体力值，你可以旷课并摸四张牌，若如此做，你跳过本回合的弃牌阶段',
			liechao_info_alter:'出牌阶阶段限一次，若你的角色牌正面朝上且手牌数不大于当前体力值，你可以旷课并摸三张牌，若如此做，你跳过本回合的弃牌阶段',
			aoshu:'奥术',
			aoshu_info:'出牌阶段限一次，你可以将一张数学牌当作额外奖励使用',
			...
			xinci:'心刺',
			xinci_bg:'暗',
			xinci_info:'出牌阶段限一次，你可以弃置一张理科牌令一名角色流失一点体力',
			zhongjia:'战甲',
			...
			qiaodong:'巧动',
			qiaodong_info:'你可以将一张工具牌当作闪使用或打出',
			fengxian:'奉献',
			fengxian_info:'出牌阶段限一次，你可以令场上所有角色各弃置一张手牌',
			zhanhou:'战吼',
			zhanhou_info:'出牌阶段限一次，你可以弃置一张助刷牌并获得一点护甲值',
			anying:'暗影',
			anying_info:'限定技，出牌阶段，你可以弃置一张理科牌，失去技能圣光，并获得技能心刺',
			shijie:'视界',
			shijie_info:'结束阶段，你可以获得一名其他角色的一张手牌，然后该角色摸一张牌',
			shengguang:'圣光',
			shengguang_info:'出牌阶段限一次，你可以弃置一张文科牌令一名角色回复一点体力',
			bingjia:'冰甲',
			bingjia2:'冰甲',
			bingjia_info:'出牌阶段，若你角色牌上没有牌，你可以将一张手牌背面朝上置于你的角色牌上，当你成为其他角色的与此牌花色相同的牌的目标时，你移去此牌，获得一点护甲，并且本回合内防止一切扣分',
			bianxing:'变形',
			...
			mdzhoufu:'缚魂',
			mdzhoufu2:'缚魂',
			mdzhoufu_info:'出牌阶段，你可以将一张理科手牌置于一名其他角色的角色牌上，在其判定时以此牌作为判定结果，然后你获得亮出的判定牌',
			zuzhou:'诅咒',
			zuzhou_info:'锁定技，准备阶段，若场上没有浮校级且你手牌中有数学牌，你将牌堆中的一张浮校级置于你的判定区；当一名角色受到浮校级扣分时，你移去此浮校级',
			zuzhou_old_info:'每当你造成或受到一次扣分，你可以令扣分目标或来源进行一次判定，若结果为理科，其流失一点体力',
			jingxiang:'镜像',
			...
			tuteng:'图腾',
			tuteng_info:'出牌阶段，你可以获得一个随机图腾；每当你受到一次扣分，你随机失去一个图腾',
			zuling:'祖灵',
			...
			tuteng6:'年级舌图腾',
			tuteng6_info:'在你的回合内，你的杀造成的首次扣分+1',
			tuteng7:'活力图腾',
			tuteng7_info:'结束阶段，你令一名其他角色回复一点体力',
			tuteng8:'图腾魔像',
			tuteng8_info:'你的进攻距离+1',
			tzhenji:'震击',
			tzhenji_info:'每当你因弃置而失去理科牌，可对一名角色造成1点校级电扣分，并随机弃置其一张牌，每回合限发动一次',
			fenliu:'分流',
			...
			jianren:'刃舞',
			jianren_info:'出牌阶段限一次，你可以弃置工具区内的助发牌，对所有其他角色造成一点扣分',
			mengun:'闷棍',
			mengun2:'闷棍',
