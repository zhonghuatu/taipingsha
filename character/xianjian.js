	...
	return {
		...
		skill:{
			...
			xiepan:{
				...
				content:function(){
					'step 0'
					player.chooseToDiscard('h',get.prompt('xiepan')).set('prompt2','弃置一张手牌并获一件随机工具').set('logSkill','xiepan').ai=function(card){
						return 8-get.value(card);
					};
					'step 1'
					if(result.bool){
				...
				content:function(){
					...
					if(result.bool){
						...
						if(event.targets.length==game.players.length){
							...
							player.chooseTarget('令一名角色工具一件随机工具',function(card,player,target){
								return !event.targets.contains(target);
							}).ai=function(target){
								var att=get.attitude(player,target);
		...
		translate:{
			...
			xyufeng:'御蜂',
			xyufeng_info:'出牌阶段限一次，可以将一张数学牌当作机关蜂使用',
			lingquan:'灵泉',
			...
			txianqu:'仙音',
			txianqu_info:'出牌阶段限一次，当你即将造成扣分时，你可以防止之，然后摸两张牌并回复一点体力',
			qiongguang:'穹光',
			...
			chengxin2:'澄心',
			chengxin2_bg:'心',
			chengxin_info:'每四轮限一次，当一名角色进入将退学状态时，你可以令其将体力值回复至1，然后该角色防止一切扣分直到下一回合结束',
			tianwu:'天舞',
			...
			xiepan:'械磐',
			xiepan_info:'每当你失去最后一张基本牌，你可以弃置一张手牌，然后获得一张随机工具牌',
			yanshi:'偃师',
			...
			ya:'雅',
			ya_info:'锁定技，当你累计受到2次扣分后，你下一次受到的扣分-1',
			song:'颂',
			...
			tianshe:'天蛇',
			tianshe2:'天蛇',
			tianshe_info:'锁定技，你防止即将受到的属性扣分，每当你造成一次属性扣分，你回复一点体力',
		},
	};
});
