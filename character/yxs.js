     ...
     forced:true,
          ...
          var target=player.storage.yxs_menshen2;
			...
			wlianhuan:{
				...
				content:function(){
					'step 0'
					var next=player.chooseToDiscard('e',get.prompt('wlianhuan',trigger.player),'弃置一张工具区内的牌使扣分+1');
			...
			lshengong:{
				...
				content:function(){
					...
					if(result.links[0]){
						...
						event.card=cards[0];
						player.chooseTarget('选择一个角色工具'+get.translation(result.links),function(card,player,target){
							return !target.isMin();
						}).ai=function(target){
		...
		translate:{
			...
			lguiyin:'归隐',
			lguiyin_info:'出牌阶段，若你本回合内未造成扣分，你可以摸一张牌，并转变为汉丁顿伯爵（每回合只能转变一次）',
			lzhangyi:'仗义',
			...
			lshengong:'神工',
			lshengong_info:'出牌阶段限一次，你可以选定场上任意一名角色的工具区的非特殊牌，出自己的一张手牌复制该工具，然后可以选择工具上自己或者别的角色的工具区',
			zhexian:'谪仙',
