	...
	return {
		...
		start:function(){
			...
			if(!lib.config.new_tutorial){
				...
				var step1=function(){
					ui.create.dialog('欢迎来到太平杀，是否进入新手向导？');
					...
					ui.create.control('在菜单中，可以进行各项设置',function(){
						...
						ui.controls[0].replace('如果你感到游戏较卡，可以开启流畅模式',function(){
							ui.controls[0].replace('在技能一栏中，可以设置自动发动或双将禁配的技能',function(){
								ui.click.menuTab('武将');
								ui.controls[0].replace('在武将或卡牌一栏中，单击角色/卡牌可以将其禁用',function(){
									ui.click.menuTab('战局');
			...
			chooseCharacter:function(){
				...
				next.setContent(function(){
					...
					ui.create.cheat2=function(){
						ui.cheat2=ui.create.control('自由选角色',function(){
		...
		translate:{
			...
			identity_dajiang_bg:'汉',
			identity_zeishou_bg:'基',
			identity_junshi:'智囊',
			identity_dajiang:'壮汉',
			identity_zeishou:'阿基米德',
			ai_strategy_1:'均衡',
			ai_strategy_2:'抬杠',
			ai_strategy_3:'好学',
			ai_strategy_4:'酱油',
					...
					begin:{
						...
						content:function(){
							...
							if(!_status.connectMode){
								...
								player.chooseControl('ok').set('dialog',[get.translation(target)+'是杠精',[[target.name],'character']]);
							}
						}
					},
					log:{
