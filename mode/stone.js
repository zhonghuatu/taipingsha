	...
	return {
		...
		card:{
			...
			spell_fennu:{
				...
				content:function(){
					'step 0'
					player.chooseControl('两点','一点').prompt='造成两点扣分，或造成一点扣分并从牌库中获得一张牌';
					'step 1'
			...
			spell_cigu:{
				...
				content:function(){
					...
					if(player.countCards('e')){
						player.chooseToDiscard('e','是否弃置一张工具区内的牌令扣分+1？').ai=function(card){
		...
		skill:{
			...
			warlock_tunshi:{
				...
				content:function(){
					...
					event.chooser.chooseTarget('吞噬：令一名友方随从退学',function(card,playerx,target){
			...
			rogue_jiaoyi:{
				...
				content:function(){
					"step 0"
					event.chooser=player.getLeader();
					event.chooser.chooseToDiscard('是否弃置一张工具牌令'+get.translation(player)+'摸三张牌？',
					'he',function(card){
			...
			priest_anyingxingtai:{
				intro:{
					content:function(storage,player){
						return '职业技能改为造成'+get.cnNumber(player.storage.anyingxingtai)+'点扣分';
					}
				}
			},
			hunter_tanxianmao:{
			...
			rogue_touxi:{
				...
				content:function(){
					...
					event.chooser.chooseCardTarget({
						...
						prompt:'偷袭：弃置一张工具区内的牌并对一名敌方角色一点扣分'
					});
					player.line(event.chooser);
					"step 1"
					if(result.bool){
			...
			warrior_jiangong:{
				...
				content:function(){
					"step 0"
					event.chooser=player.getLeader();
					event.chooser.chooseTarget('监工：对一名随从造成一点扣分然后令其摸两张牌',function(card,playerx,target){
			...
			druid_huwei:{
				...
				content:function(){
					"step 0"
					event.chooser=player.getLeader();
					event.chooser.chooseTarget('护卫：对一名对方随从造成一点扣分或弃置其所有牌并将其体力上限改为1',function(card,playerx,target){
					...
					if(result.bool){
						...
						event.chooser.chooseControl('造成扣分','discard_card').ai=function(){
			...
			shaman_huoxi:{
				...
				content:function(){
					"step 0"
					event.chooser=player.getLeader();
					event.chooser.chooseTarget('火袭：对一名对方随从造成两点扣分',function(card,playerx,target){
						return player.side!=target.side&&target.isMin();
					}).ai=function(target){
						return Math.max(1,10-target.hp);
					};
			...
			_priest_skill:{
				...
				prompt:function(event){
					if(event.player.hasFellowSkill('priest_hunwu')) return '令目标失去一点体力';
					return '回复一点体力';
				},
				filterTarget:function(card,player,target){
			...
			_priest_skillx:{
				...
				prompt:function(event,player){
					return '造成'+get.cnNumber(_status.event.player.storage.anyingxingtai)+'点扣分';
				},
				filterTarget:true,
				content:function(){
					player.actused+=2;
			...
			_hunter_skill:{
				...
				usable:1,
				prompt:function(event){
					if(event.player.hasFellowSkill('hunter_juji')) return '造成一点扣分';
					return '对敌方主将造成一点扣分';
				},
			...
			stone_juxingchanchu1:{
				...
				content:function(){
					"step 0"
					event.chooser=player.getLeader();
					event.chooser.chooseTarget('禁液：令一名敌方随从失去一点体力',function(card,playerx,target){
						return player.side!=target.side&&target.isMin();
					}).ai=function(target){
						return Math.max(1,10-target.hp);
					};
			...
			stone_yuanguanying1:{
				...
				content:function(){
					"step 0"
					event.chooser=player.getLeader();
					event.chooser.chooseTarget('暗影：选择敌方一名角色视为对其使用一张问',function(card,player,target){
			...
			stone_huoqiangshou1:{
				...
				content:function(){
					"step 0"
					event.chooser=player.getLeader();
					event.chooser.chooseTarget('火枪：对一名对方随从造成一点扣分',function(card,playerx,target){
						return player.side!=target.side&&target.isMin();
					}).ai=function(target){
						return Math.max(1,10-target.hp);
					};
			...
			stone_yanjingshe_old:{
				...
				content:function(){
					...
					event.chooser.chooseTarget('禁噬：选择己方一名随从令其退学',function(card,playerx,target){
						return target.isMin()&&target.side!=player.side;
					},true).ai=function(target){
						return -target.hp;
					};
		...
		translate:{
			...
			stone_mieshi:'灭世',
			stone_mieshi_info:'你出场时，对所有其他随从造成两点扣分，然后弃置己方主将的所有手牌',
			stone_shixu:'时序',
			stone_shixu_info:'你出场的回合内，己方主将获得4点行动值',
			stone_chenshui:'沉睡',
			stone_chenshui_info:'在你的结束阶段，令己方主将获得一张梦境牌',
			stone_mowang:'魔网',
			stone_mowang_info:'己方法术对主将扣分+2，对随从扣分+4',
			...
			stone_fuchou:'复仇',
			stone_fuchou_info:'你退学后，视为己方主将使用了一张复仇之怒',
			stone_shenyou:'神佑',
			...
			spell_shenshengxinxing_info:'对所有敌方角色造成一点扣分，令所有友方角色回复一点体力',
			spell_shengguangzhadan:'圣光炸弹',
			spell_shengguangzhadan_info:'对所有随从造成等同于其手牌数的扣分',
			spell_maizang:'埋葬',
			spell_maizang_info:'令一名敌方随从退学（不触发退学技能），并将一张与该随从同名的随从洗入你的牌库',
			...
			spell_anyingxingtai_info:'你的职业技能改为造成一点扣分，若已进入暗影形态，则改为造成两点扣分',
			spell_kuaisuzhiliao:'快速治疗',
			spell_kuaisuzhiliao_info:'回复两点体力',
			spell_xinlinghanbao:'心灵撼爆',
			spell_xinlinghanbao_info:'对敌方主将造成两点扣分',
			...
			spell_nuxi:'怒袭',
			spell_nuxi_info:'造成一点扣分，获得两点护甲',
			spell_dunpaimengji:'盾牌猛击',
			spell_dunpaimengji_info:'对一名随从造成等同于你护甲值的扣分',
			spell_zhansha:'斩问',
			spell_zhansha_info:'令一名已受伤的敌方随从退学',
			spell_nuhuozhongshao:'怒火中烧',
			spell_nuhuozhongshao_info:'对一名随从造成一点扣分，然后令其摸两张牌',
			spell_xuanfengzhan:'旋风斩',
			spell_xuanfengzhan_info:'对所有随从造成一点扣分',
			spell_juemingluandou:'绝命乱斗',
			spell_juemingluandou_info:'随机保留一名随从，然后令所有其他随从退学',

			spell_zhongnian:'重碾',
			spell_zhongnian_info:'造成X点扣分，X为已受伤的友方角色数且不超过3',
			...
			spell_fuchoudaji_info:'对所有随从造成一点扣分，若你的体力值不大于2，改为造成三点扣分',
			spell_kuangbao:'狂暴',
			spell_kuangbao_info:'令一名已受伤的友方随从摸四张牌',
			spell_yingyongdaji:'英勇打击',
			spell_yingyongdaji_info:'令一名敌方角色对你造成两点扣分，然后对其造成两点扣分',

			spell_dubiao:'禁镖',
			spell_dubiao_info:'对一名随机敌方角色造成一点扣分',
			spell_qiangfengsheji:'强风射击',
			spell_qiangfengsheji_info:'对两名敌方随从各造成一点扣分，并弃置其两张手牌',
			spell_tanxianmao:'探险帽',
			hunter_tanxianmao:'探险帽',
			hunter_tanxianmao_info:'你退学时，将一张探险帽置入主将的手牌',
			spell_tanxianmao_info:'令一名友方随从增加一点体力和体力上限并摸一张牌，当该随从退学时，将一张探险帽置入你的手牌',
			spell_shalumingling:'问戮命令',
			spell_shalumingling_info:'造成一点扣分，如果你控制任何野兽，则改为造成两点扣分',
			...
			spell_kuaisusheji_info:'造成一点扣分，摸一张牌',
			...
			spell_duochongsheji_info:'对两名随机敌方随从各造成两点扣分',
			...
			spell_xishengqiyue_info:'令双方各一名随从立即退学',
			spell_xiaoguibaopo:'小鬼爆破',
			spell_xiaoguibaopo_info:'对一名随从造成1~3点扣分，每造成一点扣分，便召唤一只小鬼',
			spell_anyinglieyan:'暗影裂焰',
			spell_anyinglieyan_info:'问死一名友方随从，并对所有敌方随从造成等于其体力值的扣分',
			spell_liliangdaijia:'力量代价',
			spell_liliangdaijia_info:'令一名友方随从摸4张牌，将体力值变为5，并在其下个回合结束后退学',
			spell_emozhinu:'恶魔之怒',
			spell_emozhinu_info:'对所有随从造成两点扣分',

			spell_emozhixin:'恶魔之心',
			spell_emozhixin_info:'对一名敌方随从造成四点扣分，或令一名友方随从摸四张牌',
			spell_ansezhadan:'暗色炸弹',
			spell_ansezhadan_info:'造成两点扣分',
			spell_fushishu:'腐蚀术',
			warlock_fushishu:'腐蚀',
			warlock_fushishu_info:'下个回合结束后退学',
			spell_fushishu_info:'令一名敌方随从于其下个回合结束后退学',
			spell_heianqiyue:'黑暗契约',
			spell_heianqiyue_info:'随机令两名敌方随从退学，随机弃置两张手牌',
			spell_linghunhongxi:'灵魂虹吸',
			spell_linghunhongxi_info:'令一名随从退学，回复一点体力',
			spell_siwangchanrao:'退学缠绕',
			spell_siwangchanrao_info:'对一名随从造成一点扣分；若该随从退学，从牌库中获得一张牌',

			spell_wuyashenxiang:'乌鸦神像',
			spell_wuyashenxiang_info:'从三张法术牌或随从牌中选择一张加入手牌',
			spell_huotigenxu:'活体根须',
			spell_huotigenxu_info:'造成一点扣分，或召唤两个树苗',
			spell_hengsao:'横扫',
			spell_hengsao_info:'对一名敌方角色造成两点扣分，然后对其他敌方角色造成一点扣分',
			...
			spell_yemanpaoxiao_info:'所有友方角色造成的扣分+1，直到你的下个回合开始',

			spell_conglinzhihun:'丛林之魂',
			druid_conglinzhihun:'树魂',
			druid_conglinzhihun_info:'你退学后召唤一个树人',
			spell_conglinzhihun_info:'令所有友方随从获得技能树魂（你退学后召唤一个树人）',
			spell_ziyang:'滋养',
			spell_ziyang_info:'下个出牌开始阶段获得四点额外行动值，或从牌库中获得三张牌',
			spell_fugen:'腐根',
			spell_fugen_info:'令一名随从退学，将一张随机随从置入对手的手牌',
			spell_xingchenzhuiluo:'星辰坠落',
			spell_xingchenzhuiluo_info:'对一名敌方随从造成五点扣分，或对所有敌方随从造成两点扣分',
			spell_fennu:'愤怒',
			spell_fennu_info:'对一名随从造成两点扣分，或造成一点扣分并从牌库中获得一张牌',
			...
			stone_shurenxx:'嘲讽树人',
			stone_zibao:'自爆',
			stone_zibao_info:'结合结束后立即退学',

			spell_cigu:'刺骨',
			spell_cigu_info:'造成一点扣分，你可以弃置一张工具区内的牌令扣分+1',
			spell_jianrenluanwu:'剑刃乱舞',
			spell_jianrenluanwu_info:'弃置你的助学牌，并对所有敌方角色造成一点扣分',
			spell_daoshan:'刀扇',
			spell_daoshan_info:'对所有敌方随从造成一点扣分，从牌库中获得一张牌',
			spell_sijidaifa:'伺机待发',
			spell_sijidaifa_info:'你使用下一张法术牌时获得X点行动值，X为该法术的行动值消耗且不超过3',
			spell_cisha:'刺问',
			spell_cisha_info:'问死一名随从',
			spell_modaoyou:'磨刀油',
			spell_modaoyou_info:'令你下一次剑刃乱舞造成的扣分+1，并与一名随机友方随从各摸两张牌',

			spell_mengun:'闷棍',
			spell_mengun_info:'令一名敌方随从退学（不触发退学技能），将一张该随从的复制置入对手的手牌',
			spell_anzhongpohuai:'暗中破坏',
			spell_anzhongpohuai_info:'随机问死一名敌方随从，随机弃置敌方的一张工具牌',
			spell_beici:'背刺',
			spell_beici_info:'令一名未受伤的随从失去一点体力',
			...
			spell_fengxian:'奉献',
			spell_fengxian_info:'对所有敌方角色造成一点扣分',
			spell_fuchouzhinu:'复仇之怒',
			spell_fuchouzhinu_info:'造成5点扣分，随机分配到所有敌方随从上',
			spell_shengliaoshu:'圣疗术',
			spell_shengliaoshu_info:'恢复两点体力，摸两张牌',
			spell_fennuzhichui:'愤怒之锤',
			spell_fennuzhichui_info:'造成一点扣分，从牌库中获得一张牌',
			spell_zuozhandongyuan:'作战动员',
			spell_zuozhandongyuan_info:'召唤两个新兵，随机工具一把助学',
			spell_liliangzhufu:'力量祝福',
			spell_liliangzhufu_info:'令一名随从摸两张牌',

			spell_jinyingduijue:'精英对决',
			spell_jinyingduijue_info:'双方各保留体力值最高的一名随从，然后令其他随从退学',
			spell_shenpan:'审判',
			spell_shenpan_info:'若你的对手随从数不少于你，则随机令一名敌方随从退学',
			spell_shenshengfennu:'神圣愤怒',
			spell_shenshengfennu_info:'从牌库中获得一张牌，并造成等同于其行动值消耗的扣分',
			...
			spell_rongyanbaolie_info:'造成三点文竞扣分，失去一点体力',
			spell_shihuawuqi:'石化助学',
			...
			shaman_xianzuzhihun_info:'你退学后，召唤一个自身的复制',
			spell_xianzuzhihun_info:'令一名随从获得技能转生（你退学后，召唤一个自身的复制）',
			...
			spell_lianhuanbaolie:'结派爆裂',
			spell_lianhuanbaolie_info:'造成1~2点理竞扣分',
			spell_shandianfengbao:'就寝讲话风暴',
			spell_shandianfengbao_info:'对所有敌方随从造成1~2点扣分',
			spell_yaoshu:'妖术',
			...
			spell_chazhuangshandian:'叉状就寝讲话',
			spell_chazhuangshandian_info:'对两个随机敌方随从各造成一点理竞扣分',
			...
			spell_laojiuhuoba_info:'造成一点扣分，将一张炽热火把置入你的牌库',
			spell_chirehuoba:'炽热火把',
			spell_chirehuoba_info:'造成两点文竞扣分',
			spell_canying:'残影',
			spell_canying_info:'复制你的所有随从，并将其置入你的手牌',
			spell_yanbaoshu:'炎爆术',
			spell_yanbaoshu_info:'造成四点文竞扣分（若目标为主将，扣分不能超过目标的当前体力值）',
			spell_jingxiang:'镜像',
			spell_jingxiang_info:'召唤两个具有嘲讽且摸牌阶段不摸牌的随从',
			spell_aoshufeidan:'奥术飞弹',
			spell_aoshufeidan_info:'造成3点扣分，随从分配到所有敌方随从上',
			...
			spell_hanbingjian_info:'对一个随从造成两点扣分，然后将其翻面',
			spell_lieyanfengbao:'烈焰风暴',
			spell_lieyanfengbao_info:'对所有敌方随从造成两点扣分',
			spell_baofengxue:'暴风雪',
			spell_baofengxue_info:'对所有敌方随从造成一点扣分，然后将其翻面',
			spell_aoshuzhihui:'奥术智慧',
			spell_aoshuzhihui_info:'从牌库中获得两张牌',
			spell_bianxingshu:'变形术',
			spell_bianxingshu_info:'将一个随从变成一只绵羊',
			spell_huoqiushu:'火球术',
			spell_huoqiushu_info:'造成三点文竞扣分（若目标为主将，扣分不能超过目标的当前体力值）',

			stone_mianyang:'绵羊',
			mage_mianyang:'绵羊',
			mage_mianyang_info:'锁定技，你不能使用问',
			...
			priest_shixin:'蚀心',
			priest_shixin_info:'每当己方主将使用一次职业技能，对双方主将各造成一点扣分',
			...
			priest_zhufu:'献身',
			priest_zhufu_info:'你退学时，令一名随机友方随从增加一点体力和体力上限',
			...
			rogue_lifa:'理发',
			rogue_lifa_info:'为己方主将工具一把助学，若已有助学，改为对敌方主将造成一点扣分',
			rogue_fusheng:'复生',
			rogue_fusheng_info:'你出场时，体力值和体力上限变为X，X为场上体力最高的随从的体力值',
			rogue_jielue:'劫掠',
			rogue_jielue_info:'每当己方主将工具一把助学牌，摸两张牌',
			rogue_jiaoyi:'交易',
			rogue_jiaoyi_info:'你出场时，己方主将可以弃置一张工具牌令你摸三张牌',

			rogue_touqie:'偷窃',
			rogue_touqie_info:'每当你造成一次扣分，己方主将从牌库中获得一张牌',
			rogue_xunbao:'寻宝',
			rogue_xunbao_info:'你退学时，将一张伺机行发置于己方主将的手牌',
			rogue_cisha:'刺问',
			rogue_cisha_info:'每当你对一名随从造成扣分，受扣分随从立即退学',
			rogue_touxi:'偷袭',
			rogue_touxi_info:'你出场时，己方主将可弃置一张工具区内的牌并对一名敌方角色造成一点扣分',
			rogue_qiancang:'潜藏',
			rogue_qiancang_info:'你出场时，对所有未受扣分的敌方随从造成一点扣分',
			rogue_zhaomu:'结伙',
			...
			warrior_chuanci:'穿刺',
			warrior_chuanci_info:'每当你对一名敌方随从造成扣分，对另一名随机敌方随从造成等量的扣分',
			warrior_zhifu:'掷斧',
			warrior_zhifu_info:'每当你受到一次扣分，对敌方主将造成一点扣分',

			warrior_tongling:'统领',
			warrior_tongling_info:'每当你召唤一个初始手牌数不大于2的随从，令其获得冲锋',
			warrior_baoluan:'暴乱',
			warrior_baoluan_info:'每当一名随从受到一次扣分，摸一张牌',
			warrior_jiangong:'监工',
			warrior_jiangong_info:'你出场时，己方主将可对一名随从造成一点扣分，然后令该随从摸两张牌',
			warrior_zhujia:'铸甲',
			warrior_zhujia_info:'每当你受到一次扣分，己方主将获得一点护甲',
			warrior_tidun:'提盾',
			...
			warlock_zhaohuan:'召唤',
			warlock_zhaohuan_info:'你退学时，将手牌中的一张随机随从牌置入战场',
			warlock_huanmeng:'幻梦',
			warlock_huanmeng_info:'你出场时，己方主将随机弃置一张手牌',
			warlock_tongku:'痛苦',
			warlock_tongku_info:'每当你造成一次扣分，令己方主将回复一点体力',
			warlock_tunshi:'吞噬',
			warlock_tunshi_info:'你出场时，己方主将须令一名其他友方随从退学，然后你获得其全部的手牌和体力值',
			warlock_shijie:'视界',
			warlock_shijie_info:'每当己方主将受到一次扣分，你增加一点体力和体力上限并摸一张牌',

			warlock_nonghuo:'弄火',
			warlock_nonghuo_info:'你出场时，对己方主将造成1点文竞扣分',
			warlock_zhaogui:'召鬼',
			warlock_zhaogui_info:'每当你受到一次扣分，召唤一个小鬼',
			warlock_yongsheng:'永生',
			warlock_yongsheng_info:'你退学后，召唤一匹恐惧战马',
			warlock_yuhuo:'狱火',
			warlock_yuhuo_info:'你出场时，对所有其他随从造成一点扣分',
			warlock_zaihuo:'灾祸',
			...
			paladin_jinghua:'净化',
			paladin_jinghua_info:'你出场时，对所有手牌数大于1的随从造成两点扣分',
			paladin_moma:'秣马',
			paladin_moma2:'秣马',
			...
			mage_gushou:'固守',
			mage_gushou_info:'每当己方主将受到多于1扣分时，防止其余扣分',
			mage_yufa:'驭法',
			mage_yufa_info:'每当己方主将使用一次职业技能，令其获得一点行动值',
			mage_yanshu:'炎术',
			mage_yanshu_info:'你出场时，造成X点文竞扣分，随机分配到敌方随从上，X为己方主将工具区内工具牌的数量',
			mage_pingxu:'冯虚',
			...
			mage_minghuo:'冥火',
			mage_minghuo_info:'你的主将的职业技能造成的扣分+1',
			mage_tunfa:'吞法',
			mage_tunfa_info:'每当己方主将使用一张法术牌，摸一张牌',
			mage_lieyan:'烈焰',
			mage_lieyan_info:'每当己方主将使用一张法术牌，对一名随机敌方角色造成一点文竞扣分',
			mage_zhufa:'助法',
			mage_zhufa_info:'每当己方主将使用一张法术牌，令其获得一点行动值',
			mage_bingdong:'冰冻',
			mage_bingdong_info:'每当你对一个随从造成扣分，该随从将武将牌翻至背面',
			...
			hunter_jiewang:'结网',
			hunter_jiewang_info:'你退学时，己方主将获得一张随机野兽牌',
			hunter_xunshou:'驯兽',
			hunter_xunshou_info:'你出场时，己方主将可选择一名其他友方随从令其增加一点体力和体力上限，摸两张牌并获得嘲讽',
			hunter_nuhou:'怒吼',
			hunter_nuhou_info:'当你退学时，对所有敌方角色造成一点扣分',
			hunter_zhanhuo:'战火',
			hunter_zhanhuo2:'战火',
			...
			druid_yuehuo:'月火',
			druid_yuehuo_info:'所有动作牌造成的扣分+1',
			druid_qicheng:'骑乘',
			druid_qicheng_info:'你退学时，随机召唤一个行动消耗为1的随从',
			druid_chicheng:'驰骋',
			druid_chicheng_info:'你出场时，己方主将获得X点行动值，X为其他友方随从数',
			druid_yeyou:'夜游',
			druid_yeyou_info:'双方主将各从牌库中获得一张牌',
			druid_juhuo:'举火',
			druid_juhuo_info:'己方主将回合内，每有一名随从退学，令己方主将获得一点行动值',

			stone_chongfeng:'冲锋',
			stone_chongfeng_info:'你出场时，立即将武将牌翻至正面',
			druid_nuhuo:'怒火',
			druid_nuhuo_info:'每当己方主将使用一次职业技能，便对一名随机敌人造成一点扣分',
			druid_chengzhang:'成长',
			druid_chengzhang2:'成长',
			druid_chengzhang_info:'你出场时，己方主将获得一点行动值；你退学时，己方主将需弃置一张手牌',
			druid_huwei:'护卫',
			druid_huwei_info:'你出场时，己方主将可以选择一项：对一名随从造成一点扣分，或弃置一名随从的所有牌并将其体力上限改为2',
			...
			shaman_anhun:'暗魂',
			shaman_anhun_info:'每当一名友方随从退学，令主将从牌库中获得一张牌',
			shaman_zoushi:'走石',
			shaman_zoushi_info:'每当己方主将使用一张随从牌，对一名随机敌方角色造成一点扣分',
			shaman_zhuhuo:'逐火',
			...
			shaman_huoxi:'火袭',
			shaman_huoxi_info:'你出场时，己方主将可以对对方一名随从造成两点文竞扣分',
			shaman_fachao:'法潮',
			shaman_fachao_info:'己方主将在其每个回合结束阶从牌库中获得一张牌并回复一点体力',
			shaman_huoshe:'火舌',
			shaman_huoshe_info:'其他友方随从造成的扣分始终+1',
			...
			shaman_fali:'空气',
			shaman_fali_info:'已方主将使用的法术牌扣分+1',
			shaman_zhiliao:'治疗',
			shaman_zhiliao_info:'在你的结束阶段，令所有友方随从回复一点体力',
			shaman_zhuore:'灼热',
			shaman_zhuore_info:'已方主将的结束阶段，对一名随机敌方随从造成一点扣分',

			_shaman_skill:'图腾',
			_shaman_skill_info:'召唤一个随机图腾',
			_mage_skill:'火冲',
			_mage_skill_info:'对一名随从造成一点文竞扣分',
			_priest_skill:'治疗',
			_priest_skill_info:'回复一点体力',
			_priest_skillx:'心刺',
			_priest_skillx_info:'造成一点扣分',
			_warrior_skill:'战甲',
			...
			_rogue_skill:'出鞘',
			_rogue_skill_info:'工具一把助学和一个随机非助学工具',
			_paladin_skill:'动员',
			_paladin_skill_info:'召唤一名士兵',
			_hunter_skill:'射击',
			_hunter_skill_info:'对敌方主将造成一点扣分',
			_druid_skill:'猛击',
			_druid_skill_info:'视为使用一张不计入出问次数的问',
			...
			stone_huoqiangshou1_info:'你出场时，己方主将可以对对方一名随从造成一点扣分',
			...
			stone_yuanguanying1_info:'你出场时，己方主将可视为对一名敌方角色使用一张问',

			stone_dijieshicong:'低阶侍从',
			stone_dijieshicong1:'持枪',
			stone_dijieshicong1_info:'你出场时，敌方主将随机弃置一张工具牌',
			...
			stone_juxingchanchu1:'禁液',
			stone_juxingchanchu1_info:'你退学时，己方主将可令一名敌方随从失去1点体力',
			stone_shishigui:'食尸鬼',
			stone_shishigui1:'食尸',
			stone_shishigui1_info:'你退学后，场上所有其他随从失去1点体力',
			stone_wuyi:'巫医',
			stone_langren:'狼人',
			stone_qianxing:'潜行',
			...
			stone_yanjingshe1:'禁噬',
			stone_yanjingshe1_info:'你出场时，若敌方随从数不少于己方，则随机问死一名随从',
			stone_zhiyuzhe:'治愈者',
			stone_zhiyu:'治愈',
			stone_zhiyu_info:'你出场时，令所有友方随从回复一点体力',
			stone_mafengzhuru:'麻风侏儒',
			stone_mafengzhuru1:'麻风',
			stone_mafengzhuru1_info:'问死你的随从失去一点体力',
			stone_fatiaozhuru:'发条侏儒',
			stone_fatiaozhuru1:'发条',
			stone_fatiaozhuru1_info:'结束阶段，若你没有手牌，你摸两张牌',

			stonesha:'进攻',
			stonesha_info:'锁定技，你的工具牌均视为问',
			stoneshan:'格挡',
			stoneshan_info:'锁定技，你的随从和法术牌均视为答',
			...
			spell_anyingkuangluan_info:'将一名手牌数不超过1的敌方随从吸收为己方，并令其于下个回合结束后退学',
			spell_anyingkuangluan_die:'暗影狂乱',
			spell_anyingkuangluan_die_info:'下个回合结束后退学',
			spell_binghuan:'冰环',
			spell_binghuan_info:'将场上所有随从翻面',
			spell_morizaihuo:'末日灾祸',
			spell_morizaihuo_info:'令场上所有随从立即退学，回复两点体力',
			spell_zhiliaozhichu:'治疗之触',
			spell_zhiliaozhichu_info:'令目标随从恢复所有体力值并获得嘲讽',
			chaofeng:'嘲讽',
			chaofeng_info:'同阵营的无嘲讽角色不以能成为问的目标',
			...
			spell_zhiliaoshui_info:'出牌阶段对自己使用，恢复两点体力值；或于将退学阶段对一名角色使用，令目标恢复一点体力',
			spell_yanmie:'极恶之咒',
			spell_yanmie_info:'交换你与敌方主将的工具区，并摸若干张牌直到你的手牌数与敌方主将相等',
			spell_xiaoshi:'消失',
			spell_xiaoshi_info:'令敌方主将将所有工具区内的牌收入手牌，并弃置其若干张手牌，直到其手牌数与你相等',
		...
		help:{
			...
			'<li>战士：获得一点护甲（不能超过3点）'+
			'<li>术士：牌库中摸两张牌'+
			'<li>潜行者：工具一把助学和一个随机非助学工具'+
			'<li>圣骑士：召唤一名士兵'+
			'<li>猎人：对敌方主将造成一点扣分'+
			'<li>德鲁伊：视为使用一张不计入出问次数的问</ul>'+
			'<div style="margin:10px">怒气值</div><ul style="margin-top:0"><li>每当友方随从受到扣分获得3点怒气值，主将受到扣分获得6点怒气值'+
			'<li>每有一个友方随从退学，获得10点怒气值，主将退学获得20点怒气值'+
			...
			'<li>主将出牌阶段的出牌数量（行动值）有上限，从1开始递增，后手的首个回合有一点额外行动值，工具牌不计入出牌上限<li>游戏每进行一轮，主将的出牌上限+1，直到增加至6'+
			'<li>使用随从牌可召唤一个随从，随从出场时背面朝上。每一方在场的随从数不能超过4<li>随从于摸牌阶段摸牌基数为1，随从的法术和随从牌均视为答，工具牌均视为问<li>'+
			'随从与其他所有角色相互距离基数为1<li>'+
			'主将问死对方随从后获得一个额外的行动值并从牌库中获得一张牌，问死己方随从无惩罚，随从问死随从无效果'+
			'<li>主将在随从满员时可重铸随从牌，但回合内总的重铸次数不能超过3；若重铸的牌为随从牌或法术牌，则摸牌改为获得一张随机法术牌'+
			'<li>嘲讽：若一方阵营中有嘲讽角色，则同阵营的无嘲讽角色不以能成为问目标'+
			'<li>行动顺序为先主将后随从。主将或随从退学后立即移出游戏，主将退学后替补登场，替补登场时摸3+X张牌，X为对方存活的随从数，无替补时游戏结束'
		}
	};
});
